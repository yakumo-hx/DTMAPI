...
- `ModManager` unavailable, Steam not initialized, required `GetSubscribedMods` missing/throwing, or a non-enumerable required result makes the snapshot unavailable. Workshop Validation fails closed. Player mode may retain a diagnosed sole-source compatibility fallback, but a duplicate raw directory cannot win without native authority.
