...
- `System.Boolean DolocAPI::CostItemAt(System.Int32 position, System.Int32 count, System.Boolean useBox, System.Boolean shouldEqualAsItem)` is one of the fifteen creative/no-cost targets. Doloc Town 1.00 exposes this as one four-parameter method; both the ProductNative nineteen-target transaction and optional Compatibility fifteen-target creative transaction resolve `/4`.

The current signatures are anchored to the private `24456188_test_E861E0` reverse baseline and compared with `23762374_public_C416D4`. `UseTool` and `UseItem` method bodies are identical. `EnterUICheck` changes only its native menu branch from a direct horizontal-factor write to `ClearHorizontalMoveFactor()`; the modal Prefix does not replay that native branch. `MoveSpeed` remains native motion plus equipment addition, with only the equipment member name changing case. No game type is exposed through a public DTMAPI API.
