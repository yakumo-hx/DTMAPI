...
This focused map owns the managed ActionSpeed product's eleven-Hook inventory, ProductNative state/restoration boundary, and frozen old-ABI compatibility collision rules. The two added Hooks cover the independent manual-watering animation state under the existing tool-speed setting.
...
| 3 | `DolocTown.AgentStateWater.OnEnter()` | Postfix | Capture and accelerate the role and watering-can Animators through the existing tool multiplier. |
| 4 | `DolocTown.AgentStateWater.OnExit()` | Postfix | Restore the manual-watering Animator snapshot without clearing a just-marked animal interaction during state replacement. |
| 5 | `DolocTown.AgentStateInteract.OnEnter()` | Postfix | Classify the interaction and capture/accelerate its Animator state. |
| 6 | `DolocTown.AgentStateInteract.OnExit()` | Postfix | Restore interaction animation and clear interaction markers. |
| 7 | `DolocTown.AgentStateEat.OnEnter()` | Postfix | Capture and accelerate eat/drink animation. |
| 8 | `DolocTown.AgentControllerState.UseItemContinues(float)` | Prefix | Scale reviewed continuous item-use duration. |
| 9 | `DolocTown.AgentControllerState.InteractContinues(float)` | Prefix | Scale reviewed continuous interaction duration. |
| 10 | `DolocTown.AnimalRenderer.OnInteract()` | Prefix | Mark the reviewed animal interaction before generic state entry. |
| 11 | `AgentStateBase.OnExit()` (global namespace) | Postfix | Restore any remaining action animation/marker on generic exit. |
...
- ProductNative owns action classification, speed selection, tool/manual-watering Animator snapshots, continuous-use duration scaling, automatic bottle-fill cooldown, pending-animal marker/timestamp, active summary/log dedupe and configuration values. Platform registrations and their owner roots remain Core-owned.
...
- The July migration source/runtime gate passed with six production files, nine Hooks, one product policy and seven native authorities. The 2026-08-06 manual-watering correction expands the current source contract to eleven Hooks and nine native authorities; its focused validation is owned by Update `20260806-0002`.
...
Current status: `manual-watering-implemented-source-unit-verified-pending-runtime`. Historical ActionSpeed smoke and L0-L5 receipts remain valid for their exact nine-Hook bytes but do not verify the new `AgentStateWater` path. Current-build manual watering with tool speed enabled/disabled and exact eleven-owner cleanup remains the bounded runtime gate.
...
- `docs/reviews/manual-qa/2026/20260806-0002-actionspeed-manual-watering-animation.md`
- `docs/updates/2026/20260806-0002-actionspeed-manual-watering-tool-speed.md`
