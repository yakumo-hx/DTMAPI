...
## 2026-09-10 complete 0.7.0 candidate one-hour title route

- Exact Windows Runtime r3 is from `9fb6f01820560150d530bdbfd533bb040b3dbb12`, with complete API/SDK 0.7.0, on Doloc Town 1.00.07 / Steam 25163613 / Unity Mono. Seven selected managed code probes and the existing AnimalPack content pack were loaded. Accurate identities and evidence are owned by [PN-031.a](../../updates/2026/20260909-0019-platform-release-preparation.md) and [candidate acceptance](../evidence/GAME-SMOKE/20260909-platform-release-070/README.md).
- `GAME-SMOKE/20260910-004000` used a 3720-second QA delay. Acceptance separately measured from a conservative visible-title upper bound at local 00:41:19.532 to the first native load at 01:42:05.978: at least **3646.446 seconds (60m46s)**. No UI input was sent during that interval; rejected read-only requests to an already closed author session sent no game work. A later short command run contributes no time to this interval.
- Native index 0 loaded exactly once, SaveLoaded fired and native LoadGame returned true at 01:42:08.442. Provider/Consumer shared identity and value 71, native agent/resources/owned Hook, independent Control and original unrecompiled helper ABI 73/1 passed. Consumer closed before Provider, cached API was rejected, Provider's own Hook was removed, and all eight registered owners reported remaining=0 and failures=0.
- Unity Application.quitting and normal process exit passed, with no forced close or fatal window and zero Runtime Error/Fatal entries. QA/isolation cleanup and NoNativeSave gates passed. After real installer withdrawal/restoration, 30 native archives, five committed sidecars and 16 real configuration files retained hash, length, mtime and count; original native enablement and installation were restored, QA removed and the lock released. No native archive backup/writeback was used.
- This is one bounded success for the exact candidate's continuous-title-then-first-load route. It does not establish a memory budget, cover long gameplay or other native host platforms, or prove the broader Unity/Mono GC crash class solved. ISSUE-010 remains `open`.
