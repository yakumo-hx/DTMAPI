...
Ordinary managed CodeMods and ContentPacks are discovered from Doloc Town's
official local `MODS` root (`Local.*`) or a current native-verified Workshop
install root (`Workshop.*`); they are never installed as extra BepInEx plugin
entries. The historical DTMAPI game `Mods` development directory is not a
player Runtime source. DTMAPI 0.6.0 pauses the unreleased Author SDK's public
`deploy`, `update`, `install-local`, and `source local select` mutations before
they can read a package or change game/Author state. Existing deployment
journals retain read-only status plus bounded recovery/withdraw compatibility,
and stale Local Development selections may be cleared; none of those legacy
operations creates player-load or release evidence. The five-DLL shape is the
...
| `Type=CodeMod`, `CodeModKind=Advanced` | Accepted only with the versioned SDK intent, exact Advanced reference receipt, matching tracked compilation policy and verified package identity; the currently installed game is recorded separately as `Exact`/`Drift`/`Unknown` context | Bounded managed Advanced lane. G2 proves `DTMAPI.AdvancedFixture`; eleven admitted real products are verified/closed, while MoreEquipmentSlots Branch B has verified its Product-disabled cold-recovery gate and remains publication-deferred. Product evidence is identity-specific and does not open another identity or general authoring lane. |
...
unverifiable/mismatched receipt, reference-policy or package binding must fail before:
...
The 0.6 compatibility-context amendment does not weaken those package gates.
After they pass, the Loader compares the currently installed game with the
receipt's compilation baseline and records `Exact`, `Drift`, or `Unknown`.
`Drift` and `Unknown` no longer fail before `Assembly.LoadFrom`; they enter the
same real type/Entry/product activation transaction as `Exact`. A real failure
still invokes the existing owner rollback and rejects only that product.
...
| Native references | Strict uses the hash-fixed Abstractions/netstandard payload | Advanced resolves an explicit local, hash-receipted game/Harmony reference set; omitted-kind legacy inputs may directly reference native assemblies and carry private helper DLLs without receiving Advanced guarantees | G2 proof remains synthetic-specific to build `23762374`; every admitted product has a separate product-bound exact policy, whose current or historical build is governed by the amendment below |
| Game compatibility | `MinimumGameVersion` can warn and continue for existing lanes | Advanced receipt/policy records the exact compilation baseline; the installed game projects `Exact`/`Drift`/`Unknown` and all three continue to real activation | G2's historical proof remains build `23762374`; Drift/Unknown success is product/runtime evidence, not a retroactive expansion of that proof |
| Doctor | Previously one DTMAPI CodeMod artifact kind | Versioned report exposes managed identity, declaration/provenance, placement, native risk, strict receipt compatibility, non-blocking installed-game `Exact`/`Drift`/`Unknown` context and restart policy, including legacy author ownership | Direct native references/private helpers are informational for legacy, but remain errors for explicit Strict; package provenance errors remain blocking |
| Manager/UI | Previously raw `Type` without first-class identity/risk | Rows distinguish Strict, third-party native compatibility, Advanced native risk, ContentPack host state, read-only External state and the Advanced installed-game context | Legacy rows state author-managed cleanup and restart; UI does not claim hot unload, and Drift/Unknown success creates no warning |
...
## 0.6 Current/History Policy Amendment

The 0.6 source contract keeps the frozen Author SDK `0.1.0` public-API compile
target at `0.5.5` while separating it from each exact policy's executable
Runtime floor. Registry schema 2 therefore carries
`minimumDtmApiVersion`; the SDK marker still records target `0.5.5`, while the
generated manifest records the selected policy's real minimum Runtime:

- current AutoFishing, DebugConsole, MoreEquipmentSlots and MoreSaves policies bind build `24456188` and
  require DTMAPI `0.6.0` because no accepted 0.5.5 Runtime embeds those policy
  identities;
- every other current authoring policy remains bound to build `23762374` with
  Runtime floor `0.5.5`;
- the old AutoFishing, DebugConsole, MoreEquipmentSlots and MoreSaves `23762374` policy IDs remain in a
  separate exact history registry with floor `0.5.5`. Core and Doctor embed that
  registry only to accept already generated historical receipts; Author SDK
  does not expose it for new selection, validation, build or package generation;
- current and historical policy IDs are globally unique, historical rows may
  map only to an already admitted current UniqueID, and all policy/receipt/hash
  checks remain exact and fail closed.

The exact retained 0.5.5 Core remains the negative compatibility authority for
new 24456188 packages: it must reject their `0.6.0` minimum before assembly
load. The 0.6 Core/Doctor accept both the current authoring identities and
the exact historical package identities, subject to the manifest minimum and
all existing provenance checks. This is a source/unit compatibility amendment,
not player verification, publication authorization, general Advanced authoring
or a rewrite of G2's historical `23762374` proof.
...
The product owns action timing, continuous-use scaling, tool/manual-watering
Animator acceleration, animal-interaction markers, automatic bottle-fill
scheduling and restoration. It pre-resolves and atomically installs eleven Hooks,
including the independent `AgentStateWater.OnEnter/OnExit` lifecycle under the
existing tool-speed configuration. The new product does not consume
...
The current ProductNative owner atomically installs exactly four patches across
four targets: `AgentEquipmentManager.ReloadParams` Postfix,
`AgentEquipmentManager.TryGetShieldItem(IAgentEquipmentShieldItem&)` Postfix,
`AccessoriesBar.__Init` Postfix and `AccessoriesBar.OnStartShow` Postfix. The
typed shield adapter is supplied only when the native provider returned false,
so official equipment remains first and the game's current five-parameter
`BodyController.OnAttacked` tail continues to own damage/death/hit semantics.
...
The 2026-08-05 Branch B amendment closes Review `20260730-0016`'s five
transaction findings at source level: strict mail shape, exact backpack delta,
non-missing native fingerprints, exact cold destination/fingerprint, and no
delayed count-only promotion. A further audit also requires the cold Product
recovery session to exist before the first native placement; mutation-then-
throw or unreadable immediate evidence now remains quarantined and vetoes save.
Flat schema 1--3 and pre-schema global conversion preserve the exact fields
their historical formats own and publish Product v3 atomically. Current policy
`doloctown-24456188-moreequipmentslots-v1` is bound only to
`DTMAPI.MoreEquipmentSlotsMod`, build `24456188`, its exact typed shield surface
and Runtime floor `0.6.0`; the old policy remains exact history at floor
`0.5.5`. The former single-process cold smoke remains paused because it did not
prove native save/`SaveSaved` plus a second cold no-replay process. ISSUE-021's
replacement three-process boundary passed at outer case
`MOREEQUIPMENT-COLD-RECOVERY/20260806-r5`, whose aggregate receipt is retained
at `GAME-SMOKE/20260806-015320/cold-recovery-acceptance.json`: two real native saves commit the
prepared and recovered states, then a separate `NoNativeSave` process proves
zero replay and unchanged archives/committed sidecars before whole-fixture
cleanup. No new public API, Host, source root or receipt family was introduced.
...
| SDK/build | Strict still triggers `SDK160`; declared Advanced resolves only receipt-bound compilation references | Strict bypass, missing receipt, wrong target framework and package/receipt/policy hash drift rejected; installed-game drift is not confused with package drift |
...
| Loader | Package classification finishes before assembly load; installed-game context is captured before load; legacy cold-load and private dependency resolution are exercised | Invalid Advanced package/reference authority still produces zero roots; Drift/Unknown proceeds to real activation, whose failure performs bounded owner rollback; legacy Entry failures remain owner-attributed |
| Doctor/Manager | Stable machine projection plus player wording for native risk, author ownership, restart and `Exact`/`Drift`/`Unknown` | Explicit Strict is never silently relaxed; package errors remain blocking; Drift/Unknown is Info/diagnostic context rather than Warning/Error; legacy is never presented as verified Advanced or safely hot-unloaded |
...
The Doloc Town 1.00 amendment keeps that ownership boundary and adds one bounded
ProductNative migration in MoreSaves source `1.0.1`. Its current authoring
policy is `doloctown-24456188-moresaves-v1`, its frozen SDK API compile target
remains `0.5.5`, and its executable Runtime floor is `0.6.0`; the old `23762374`
policy/surface is history-only for Core/Doctor exact receipt acceptance. The
2026-08-04 implementation used internal-index validation, content fingerprints,
whole-family preflight and rollback, and its disposable player acceptance is
retained only as historical evidence for those superseded bytes.

The 2026-08-05 user correction makes the native 1.00 filename-role conversion
the current authority. After acquiring the existing product owner lease and
before publishing archive count 12, MoreSaves requires the manager's exact
`doloc-save-{0}.data` format and resolves all indices 6--11 through native
`LocalSave.GetDataFullPath` to prove one SAVE root and the exact official names.
Each old current/`-prev`/`-bak` source is then handled independently: missing
source skips, existing current/`.prev0`/`.bak` destination preserves both paths
without overwrite, otherwise `File.Move` performs the rename. Successful moves
must leave the source absent and destination present. A move exception or failed
postcondition prevents 12 from being published for that activation; completed
renames remain and the next startup resumes from the old names still present.
No archive bytes are read, decrypted, hashed, validated, repaired or rewritten,
and no DTMAPI flag, journal or rollback is added. Configure and deferred retry
failures still share the native-six/owner-release path. Normal discovery, load,
save, copy and delete remain native after the pass. This creates no SharedNative
adapter, public API, Harmony patch, conflict UI or general Advanced authoring
lane; the frozen `ISaveSlotsApi` compatibility service and its bidirectional
owner exclusion remain unchanged. ISSUE-019 first recorded an ordinary player
cold-process observation in which the exact Local 1.0.1 DLL moved all 16
present live roles to official names byte-identically and loaded native slot 7.
Current disposable `ArchiveMutation` runs `20260806-004819` and `005111` now
revalidate those bytes with populated official #7--#12 UI, native index 6 and
11 loads, cold zero-source idempotence, clean exit, exact 18-role post-state and
...
ActionSpeed: ADMITTED THIRD PRODUCT; historical nine-target third-save runtime/deactivation passed; current eleven-target manual-watering correction is source/unit/package-verified and pending bounded runtime acceptance
...
MoreSaves 1.00 official-role correction: PLAYER-VERIFIED/RELEASE-REVALIDATED; source 1.0.1, current 24456188 policy and minimum Runtime 0.6.0 retain the existing product/compatibility ownership boundary; focused source/unit gates pass without reading archive contents; the exact official Local package passed disposable 16-role migration, populated official #7--#12 UI, native index 6 load, cold zero-source idempotence, native index 11 load and clean exit without player archive writeback at GAME-SMOKE/20260806-004819 and 005111
...
MoreEquipmentSlots: BRANCH-B-COLD-RECOVERY-VERIFIED/PUBLICATION-DEFERRED EIGHTH PRODUCT; Branch B closes Review 0016's five transaction findings in source/tests, uses the current typed native shield-provider seam, preserves exact legacy-to-Product-v3 fields, and binds current build 24456188 policy to Runtime 0.6.0. Workshop 3744059735 remains exact published 0.3.1-dtmapi and continues through the frozen IEquipmentSlotsApi/demand-loaded Compatibility path. ISSUE-021's disposable native prepare/commit/second-cold-process acceptance passed at GAME-SMOKE 20260806-015140/015232/015320; broader product publication remains independently deferred
...
authors, automatically admit a thirteenth product, implement G7, or
pre-authorize a future release.
