# Archived Goal: DTMAPI 0.3.1 Manual Regression And New Content Mod

This file is a historical audit snapshot, not an active implementation goal. It preserves a 0.3.1 handoff that was later identified by the user as a Codex automatic-versioning mistake. Do not execute this file as a new goal and do not infer future target versions from it.
...
- This file preserves the 0.3.1 regression/new-content handoff as historical context. The 0.3.1 version bump was later identified by the user as a Codex automatic-versioning mistake, so do not infer future target versions from it. Treat all old 0.2.4-0.2.9 smoke evidence as orientation only.
...
- Historical version note: the original handoff asked for a `0.3.0` to `0.3.1` bump, but the user later identified that bump as erroneous. This line is retained only to explain past drift.
...
- Do not use this archived file to bump versions. A future goal must name its own fixed target version in a new goal file.
