...
The production Bootstrap UI arms evidence capture on the first ordinary console open through `screenshotPending = !screenshotRecorded` at reviewed commit `8bf018505a01:src/DTMAPI.BepInExBootstrap/ReflectedDebugConsoleUi.cs#L214`. Its ordinary update then prepares a synthetic hover specimen and invokes screenshot capture in the open console path at `8bf018505a01:src/DTMAPI.BepInExBootstrap/ReflectedDebugConsoleUi.cs#L546-L552`.

The capture method creates `DEBUG-CONSOLE-UI/<timestamp>`, requests a Unity screenshot, writes `summary.txt`, selects a QA-specific item specimen, and publishes `Smoke.DebugConsoleScreenshot`/`Smoke.DebugConsoleModItemUi` at `8bf018505a01:src/DTMAPI.BepInExBootstrap/ReflectedDebugConsoleUi.cs#L1773-L1812`. The sample selector even gives special treatment to Workshop item `3722791728` and `mod_butter` at `8bf018505a01:src/DTMAPI.BepInExBootstrap/ReflectedDebugConsoleUi.cs#L1885-L1893`. This is QA evidence policy, not the production responsibility to render and operate the Y console.

### AnimalViewer

Production Harmony callbacks invoke `RecordAnimalViewerUiEvidence` and `RecordAnimalPanelUiEvidence` without an active-QA condition in [`DolocTownHookCallbacks.cs`](../../../../../src/DTMAPI.GameBridge.DolocTown/Hooking/DolocTownHookCallbacks.cs#L368). Once an ordinary configured AnimalViewer has renderable progress rows, the reviewed production service creates `ANIMAL-001/<timestamp>`, requests a screenshot, writes a summary, and publishes both product and `Smoke.*` statuses at `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/AnimalViewer/AnimalViewerService.cs#L277`. A second delayed screenshot/summary append remains in the same reviewed player service at `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/AnimalViewer/AnimalViewerService.cs#L330`.

### EquipmentSlots

The reviewed production equipment service still contains a QA-only evidence adapter which creates `EQUIPMENT-SLOTS-UI/<timestamp>`, captures a screenshot, writes a summary, and publishes `Smoke.NewContentEquipmentSlotsUi` at `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/EquipmentSlots/DolocTownExperimentalBridgeApi.EquipmentSlots.cs#L1750`. Current repository callers are QA scenarios, so the adapter has no demonstrated ordinary product owner.
...
- `ActionSpeedService.SuppressActionSpeedAutoFillForFixture` is reviewed production state at `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/ActionSpeed/ActionSpeedService.cs#L40` and directly bypasses the normal auto-fill path at `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/ActionSpeed/ActionSpeedService.cs#L246`. The observed non-test setter is the QA ActionSpeed case at [`ActionSpeedFixtureCase.cs:694`](../../../../../src/DTMAPI.GameBridge.DolocTown.QA/Scenarios/Fixtures/ActionSpeedFixtureCase.cs#L694).
- `DolocTownExperimentalBridgeApi.ForceMachineProductionDueForFixture` remains production state at [`DolocTownExperimentalBridgeApi.cs:106`](../../../../../src/DTMAPI.GameBridge.DolocTown/DolocTownExperimentalBridgeApi.cs#L106) and makes the machine loop treat production as due independently of native TU progression at reviewed commit `8bf018505a01:src/DTMAPI.GameBridge.DolocTown/Features/MachineProduction/DolocTownExperimentalBridgeApi.MachineProduction.cs#L708`.
