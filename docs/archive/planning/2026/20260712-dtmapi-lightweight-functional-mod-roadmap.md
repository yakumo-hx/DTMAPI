...
> 2026-07-30 历史状态快照：边界修正、G2 synthetic fixture 与十一个 Advanced/ProductNative 产品均已 verified/closed；MoreEquipmentSlots 已实现，但新版 1.0.0、迁移和发布验收已由用户延期，其五个事务 P1 保留为下版本输入。Mine 的运行周期/电力/产出已物理拆分，但借用水井贴图与运行时 2x 仍阻塞它自身公开发布；DebugConsole 已完成行为等价拆分与生命周期所有权收口，视觉/UX 重写继续延期。第十三及以后产品与 Content Host G7 仍阻断。0.5.5 采用 owning Update 的既有冻结制品；完整 Release、冻结候选包审计、Manbo 与真实旧 MoreEquipmentSlots 订阅包兼容均已有证据。实际上传目录同步、Catalog releaseStop、三语言人工操作、Steam 上传及上传后复核在该快照时仍未完成。当时的 Batch 6 生命周期现只保存在 [0.5.5 历史 annex](../../../architecture/batch6-managed-mod-identity-contract.md)；当前准入查 [managed-product admission registry](../../../architecture/managed-product-admission-registry.md)，当前公开/订阅集合查 [Product Catalog](../../../../tools/release/dtmapi-product-catalog.json)、[current subscription manifest](../../../../tools/release/current-subscription-manifest.json) 与该 manifest 的 `authority.latestReleaseUpdate`。
...
> 2026-07-21 历史状态快照：下面三项与 Batch 6 Phase 0 correction、G2 synthetic fixture、AutoFishing 相关 G3-G6 均已关闭；精确实现提交和收据只由冻结的 [Batch 6 / 0.5.5 历史 annex](../../../architecture/batch6-managed-mod-identity-contract.md) 及其当时链接的 Update 保存。它们是历史前置，不再是当前阻塞，也不需要为 D.5 重放。
...
### Phase 8（0.6 之后）：启停职责与热加载扫描轻量化

功能性 CodeMod 的启用、禁用、更新、换源、版本和顺序变化统一在冷启动采用。
官方 Mod 页面成功保存后，DTMAPI 只比较新的功能性 Mod 计划与本进程启动时
实际采用的计划：有差异则记录 `restart-required`、列出受影响 Mod 并只提示
一次；玩家把状态改回启动计划时取消待重启标记。当前进程不加载新 DLL、不
停用已运行 owner、不重算依赖，也不重建配置页。纯官方 JSON 内容继续由游戏
处理；Mod 自己明确实现的配置热更新不在此限制内。

ISSUE-017 的页面事务 Hook 仍负责辨别“打开页面的预览 reload”“关闭前
reload”和“成功持久化提交”，不能因删除提交后的完整重协调而一并删除。
实施前先测量当前 `DiscoverMods() + LoadMods(false)` 的扫描时间、分配、依赖与
cleanup 复杂度，并比较保留有限观察、完全冷启动采用和少数显式热更新能力的
实际收益。没有性能和必要性证据，不建设周期扫描、通用 DLL 热加载或新的兼容
API。这一阶段不属于 0.6.0 发布实现。
