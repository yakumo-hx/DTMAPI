...
async function loadData() {
  const [summary, methods, links] = await Promise.all([
    fetch("data/summary.json").then((response) => response.json()),
    fetch("data/methods.json").then((response) => response.json()),
...
function renderOverview() {
  ...
  els.buildLine.textContent = `${meta.game} ${meta.branch}/${meta.steamBuild} | generated ${meta.generatedAtUtc}`;
...
async function main() {
  ...
  try {
    ...
    console.error(error);
    els.buildLine.textContent = "Failed to load generated data. Serve this directory over HTTP and regenerate data if needed.";
