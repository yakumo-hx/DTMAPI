...
| Migrated Gameplay | `IActionCompletionApi` | Experimental | `ActionCompletionFeature` registers the API through `ActionCompletionService`, with `ActionCompletionHookBridge` owning the ToolCollider hook and `AgentStateLifecycleHookBridge` owning the shared interact-exit callback used for fuel/feed completion. Latest shared-lifecycle evidence is `GAME-SMOKE/20260609-172748`, which verifies `OneActionResourceHit=Passed`, `OneActionWrongTool=Passed`, `OneActionFuelFeed=Passed`, `OneActionVegetation=Passed`, `Feature.ActionCompletion = ready`, unchanged `Actions.OneActionFuelFeed = verified`, and clean exit. Resource classification, fuel/feed post-interact handling, and vegetation exception paths remain GameBridge-owned and version-sensitive, so this stays Experimental. |
