...
| Migrated Gameplay | `IActionCompletionApi` | Experimental | `ActionCompletionFeature` registers the API through `ActionCompletionService`, with `ActionCompletionHookBridge` owning the ToolCollider hook and `AgentStateLifecycleHookBridge` owning the shared interact-exit callback used for fuel/feed completion. Latest native-helper evidence is `GAME-SMOKE/20260609-174124`, which verifies `OneActionResourceHit=Passed`, `OneActionWrongTool=Passed`, `OneActionFuelFeed=Passed`, `OneActionVegetation=Passed`, `Feature.ActionCompletion = ready`, unchanged `Actions.OneActionComplete = verified` / `Actions.OneActionFuelFeed = verified`, and clean exit after `ActionCompletionService` moved to `GameBridgeNativeHelpers`. Resource classification, fuel/feed post-interact handling, and vegetation exception paths remain GameBridge-owned and version-sensitive, so this stays Experimental. |
| Migrated Gameplay | `IActionSpeedApi` | Experimental | Used by ActionSpeedMod with real gameplay smoke evidence. Latest shared-lifecycle evidence is `GAME-SMOKE/20260609-172903`, which verifies tool animation, config apply, interaction slices, auto-fill, shared `AgentState*OnExit` restore logs, `Feature.ActionSpeed = ready`, and clean exit; feature-status model evidence remains `GAME-SMOKE/20260609-141440`; earlier branch-package evidence remains `GAME-SMOKE/20260609-120153`. It still patches many interaction/animation paths and needs broader conflict/lifecycle regression coverage. |
| Migrated Gameplay | `IFishingAutomationApi` | Experimental | Used by AutoFishingMod with F6, movement cancel, auto-cast, fast Pull animation, and minigame evidence. Latest restore evidence is `GAME-SMOKE/20260609-170646`, which verifies `AutoFishingHotkey=Passed`, `AutoFishingPhase=Passed`, `AutoFishingMiniGameComplete=Passed`, `Smoke.AutoFishingAnimationSpeedRestore = experimental`, clean exit, and no fatal popup. It still owns fishing state transitions and native animator writes, so it remains Experimental. |
| Migrated Gameplay | `IItemTooltipApi` | Experimental | `FishRoeTooltipFeature` registers the API through `FishRoeTooltipService`, with `FishRoeTooltipHookBridge` owning `Items.FishRoeTooltip` item-title/detail patch status. Latest split evidence is `GAME-SMOKE/20260609-181533`, which verifies `ExperimentalHooks=Passed`, `Items.FishRoeTooltip = verified`, `Feature.FishRoeTooltip = ready`, `Smoke.FishRoeTooltip = verified`, `Smoke.ExperimentalHookExercise = verified`, AnimalViewer co-smoke, and clean exit. The public FishBreedingAssistant lookup source remains a placeholder in the shareable repo, so the smoke harness registers a smoke-only fallback provider when no public lookup/config decoration is visible; the API remains display-only and Experimental. |
