...
| Migrated Gameplay | `IActionCompletionApi` | Experimental | `ActionCompletionFeature` now registers the API through `ActionCompletionService`, with `ActionCompletionHookBridge` owning the ToolCollider hook and the shared interact-exit path still observed through the existing feature-host hook route. Latest post-merge `Refactor` evidence is `GAME-SMOKE/20260609-144458`, which verifies `OneActionResourceHit=Passed`, `OneActionWrongTool=Passed`, `OneActionFuelFeed=Passed`, `OneActionVegetation=Passed`, `Feature.ActionCompletion = ready` with internal status details, and clean exit. Resource classification, fuel/feed post-interact handling, and vegetation exception paths remain GameBridge-owned and version-sensitive, so this stays Experimental. |
