...
namespace Yuuka.DTMAPI.AutoFishing
{
    ...
    internal sealed class FishingQaDiagnostics
    {
        ...
        internal long InstantBiteCommittedCount { get; private set; }
        internal string LastNativeBiteProvenance { get; private set; } = FishingBitePreparationProvenance.None.ToString();
        ...
        internal void RecordNativeBitePrepared(FishingBitePreparationProvenance provenance)
        {
            NativeBitePreparedCount++;
            LastNativeBiteProvenance = provenance.ToString();
            if (provenance == FishingBitePreparationProvenance.OrderedCommit ||
                provenance == FishingBitePreparationProvenance.ReconciledAfterWriteFault)
            {
                InstantBiteCommittedCount++;
            }
        }
