...
namespace Yuuka.DTMAPI.AutoFishing
{
    internal sealed class FishingNativeStateCache
    {
        ...
        private Func<object, float>? getHorizontalMoveFactor;
        ...
        internal FishingNativeStateCache(FishingProductContext runtime)
        {
            ...
            Current = new FishingNativeAvailability(false, false, false, false, 0d, string.Empty);
        }
        ...
        internal FishingNativeAvailability RefreshFrame()
        {
            ...
            try
            {
                ...
                {
                    Current = new FishingNativeAvailability(false, SelectedRod != null, poolKnown && hasFishingPool, false, 0d, string.Empty);
                    return Current;
                }
                ...
                bool movementAvailable = status != null && EnsureStatusAccessor(status);
                double movement = movementAvailable ? getHorizontalMoveFactor!(status!) : 0d;
            ...
            catch (Exception ex)
            {
                ...
                Agent = null;
                SelectedRod = null;
                Current = new FishingNativeAvailability(false, false, poolKnown && hasFishingPool, false, 0d, string.Empty);
                return Current;
            }
        }
        ...
        internal void SetPoolAvailability(bool value)
        {
            ...
            Current = new FishingNativeAvailability(Current.CanCast, Current.HasSelectedRod, value, Current.HorizontalMoveFactorAvailable, Current.HorizontalMoveFactor, Current.AgentState);
        }
        ...
        private bool EnsureStatusAccessor(object status)
        {
            ...
            if (statusType == type && getHorizontalMoveFactor != null)
                return true;
            if (unavailableStatusType == type)
                return false;
            try
            {
                getHorizontalMoveFactor = FishingNativeAccessors.CreateFloatGetter(FishingNativeAccessors.FindMember(type, "HorizontalMoveFactor"));
                RecordRebuild(statusType, type);
                statusType = type;
                accessorBuilds++;
                if (getHorizontalMoveFactor == null)
                {
                    unavailableStatusType = type;
                    MarkAccessorBuildFailure("status.HorizontalMoveFactor", null);
                }
                return getHorizontalMoveFactor != null;
            }
            catch (Exception ex)
            {
                unavailableStatusType = type;
                MarkAccessorBuildFailure("status.HorizontalMoveFactor", ex);
                return false;
            }
        }
