...
namespace Yuuka.DTMAPI.AutoFishing
{
    ...
    internal sealed class FishingNativeTransactionCache
    {
        ...
        internal bool TryPrepareNativeBite(object waitState, out bool isFish, out string message)
        {
            ...
            try
            {
                ...
                if (getWaitForBite!(waitState))
                {
                    ...
                    setWaitForBite!(waitState, false);
                    setHasRolled!(waitState, true);
                    setHookProbability!(waitState, 1f);
                    setFishOnHookDuration!(waitState, ResolveFishOnHookDuration());
                    invokeFishOnHookTip?.Invoke(waitState);
                    TryRefreshFishingRenderer(body);
                }

                isFish = TryReadIsFish(body);
                message = "Native bite prepared through cached fishing transaction accessors.";
                return !getWaitForBite!(waitState);
            }
            catch (Exception ex)
            {
                MarkInvocationFailure("prepare-bite", ex);
                message = "Native bite transaction failed: " + ex.GetType().Name + ".";
                return false;
            }
        }
        ...
        private bool TryReadIsFish(object body)
        {
            object? cache = getFishingCache!(body);
            if (cache == null || !EnsureFishingCacheAccessors(cache.GetType()))
                return false;
            object? fish = getFishProto!(cache);
            return fish != null && EnsureFishProtoAccessors(fish.GetType()) && getIsFish!(fish);
        }

        private void TryRefreshFishingRenderer(object body)
        {
            try
            {
            ...
            {
                MarkInvocationFailure("fish-rod-renderer-refresh", ex);
            }
        }
