...
namespace Yuuka.DTMAPI.AutoFishing
{
    internal sealed class FishingPrimitivesService
    {
        ...
        internal FishingPrimitiveResult TryPrepareNativeBite(FishingRuntimeSession session, long observedSequence, string reason)
        {
            ...
            bool isFish = false;
            bool requiresFaultClose = false;
            FishingBitePreparationProvenance provenance = FishingBitePreparationProvenance.None;
            string message = "Fishing automation runtime service is unavailable.";
            if (!nativeAdapter.TryPrepareNativeBite(
                waitState,
                out isFish,
                out requiresFaultClose,
                out provenance,
                out message))
            {
                return Reject(
                    session,
                    requiresFaultClose ? "native-bite-fault-closed" : "native-bite-failed",
                    message);
            }
            if (provenance == FishingBitePreparationProvenance.None)
                return Reject(session, "native-bite-provenance-missing", "Native bite preparation succeeded without an exact provenance classification.");
            qaDiagnostics?.RecordNativeBitePrepared(provenance);
            session.Advance(FishingPrimitivePhase.BiteReady, FishingPrimitiveTransitionKind.BiteReady, reason, isFish);
            return new FishingPrimitiveResult(true, "bite-ready-" + provenance.ToString().ToLowerInvariant(), message, session.Sequence);
        }
    ...
    internal sealed class FishingRuntimeSession
    {
        ...
        internal FishingPrimitiveSnapshot BuildSnapshot(FishingNativeAvailability native)
        {
            return new FishingPrimitiveSnapshot(
                ...
                native.NativeMovementAvailable,
                native.NativeInputMultiplier,
                native.NativeVelocityX,
                native.NativeOffsetX);
        }
