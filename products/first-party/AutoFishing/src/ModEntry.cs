...
namespace Yuuka.DTMAPI.AutoFishing
{
    public sealed class ModEntry : DtmMod, IDisposable
    {
        ...
        private bool movementCancelArmed;
        ...
        private void OnUpdateTicked(object sender, UpdateTickedEventArgs e)
        {
            ...
            FishingNativeMovementGateResult movement = EvaluateMovement(snapshot);
            movementCancelArmed = movement.Armed;
            if (movement.ShouldCancel)
            {
                SetAutomation(false, movement.Reason);
                return;
            }
            if (movement.DeferActions)
            {
                lastReason = movement.Reason;
                return;
            }
            if (movement.Reason.Length > 0)
                lastReason = movement.Reason;
            ...
            if (!result.Applied && result.Status.Equals("native-bite-fault-closed", StringComparison.OrdinalIgnoreCase))
                SetAutomation(false, result.Status);
        }
        ...
        private FishingNativeMovementGateResult EvaluateMovement(FishingPrimitiveSnapshot snapshot)
        {
            return FishingNativeMovementPolicy.Evaluate(
                movementCancelArmed,
                snapshot.NativeMovementAvailable,
                snapshot.NativeInputMultiplier,
                snapshot.NativeVelocityX,
                snapshot.NativeOffsetX);
        }
