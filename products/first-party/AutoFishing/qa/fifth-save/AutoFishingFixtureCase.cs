...
namespace DTMAPI.GameBridge.DolocTown
{
    internal sealed partial class QaScenarioController
    {
        ...
        private DateTimeOffset autoFishingMovementCancelReadyAt = DateTimeOffset.MinValue;
        private bool autoFishingMovementCancelAwaitingNativeInput;
        private string autoFishingMovementCancelOwnerId = string.Empty;
        ...
        {
            bool syntheticMovementPressed = false;
            bool terminal = false;
            try
            {
                ...
                patcher ??= new HarmonyReflectionPatcher(runtime);
                Type? dolocApi = patcher.ResolveType("DolocAPI, Assembly-CSharp");
                if (dolocApi != null && !TryGetStaticBoolProperty(dolocApi, "IsNormalState"))
                {
                    LogAutoFishingPending("Waiting for NormalGameState before auto-fishing movement-cancel smoke. context=" + runtime.UI.InputContext + ".");
                    return FixtureAttemptResult.Pending;
                }
                if (runtime.UI.BlocksGameplayHotkeys)
                {
                    LogAutoFishingPending("Waiting for gameplay hotkeys before auto-fishing movement-cancel smoke. context=" + runtime.UI.InputContext + ", menuOpen=" + runtime.UI.IsOpen + ".");
                    return FixtureAttemptResult.Pending;
                }

                FishingRuntimeSession? primitiveSession = fishingAutomationFeature?.Primitives.TryGetActiveSession();
                if (autoFishingMovementCancelAwaitingNativeInput && primitiveSession == null)
                {
                    string nativeSummary = "owner=" + autoFishingMovementCancelOwnerId + ", source=native HorizontalMoveFactor.";
                    runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingMovementCancel OK " + nativeSummary);
                    runtime.SetHookStatus("Smoke.AutoFishingMovementCancel", "verified", "Agent.Status.HorizontalMoveFactor", nativeSummary);
                    ResetAutoFishingMovementCancelCaseState();
                    terminal = true;
                    return FixtureAttemptResult.Succeeded;
                }
                if (primitiveSession == null)
                {
                    DispatchQaInputTap(toggleButton);
                    runtime.Update();
                    autoFishingHotkeyInjected = true;
                    primitiveSession = fishingAutomationFeature?.Primitives.TryGetActiveSession();
                }
                if (primitiveSession != null)
                {
                    string primitiveOwner = primitiveSession.OwnerId;
                    if (autoFishingMovementCancelAwaitingNativeInput)
                        return FixtureAttemptResult.Pending;
                    if (autoFishingMovementCancelReadyAt == DateTimeOffset.MinValue)
                    {
                        autoFishingMovementCancelReadyAt = DateTimeOffset.Now.AddMilliseconds(1100);
                        runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingHotkey OK owner=" + primitiveOwner + ", trigger=" + toggleButton + ", source=typed-keybind-frame.");
                    }
                    if (DateTimeOffset.Now < autoFishingMovementCancelReadyAt)
                        return FixtureAttemptResult.Pending;
                    FishingRuntimeSessionObservation primitiveSnapshot = primitiveSession.CaptureObservation();
                    if (primitiveSnapshot.HorizontalMoveFactorAvailable)
                    {
                        autoFishingMovementCancelAwaitingNativeInput = true;
                        autoFishingMovementCancelOwnerId = primitiveOwner;
                        runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingMovementCancel awaiting native input owner=" + primitiveOwner + ", source=Agent.Status.HorizontalMoveFactor.");
                        runtime.SetHookStatus("Smoke.AutoFishingMovementCancel", "pending", "Agent.Status.HorizontalMoveFactor", "awaiting external physical movement");
                        return FixtureAttemptResult.Pending;
                    }
                    syntheticMovementPressed = true;
                    DispatchQaInputFrame("A", isDownNow: true, pressedEdge: true, releasedEdge: false);
                    runtime.Update();
                    DispatchQaInputFrame("A", isDownNow: false, pressedEdge: false, releasedEdge: true);
                    if (fishingAutomationFeature?.Primitives.HasActiveSession == true)
                        throw new InvalidOperationException("First-party AutoFishing remained enabled after A snapshot fallback. owner=" + primitiveOwner + ".");
                    string fallbackSummary = "owner=" + primitiveOwner + ", cancel=A, source=local snapshot fallback because HorizontalMoveFactor was unavailable.";
                    runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingMovementCancel OK " + fallbackSummary);
                    runtime.SetHookStatus("Smoke.AutoFishingMovementCancel", "verified", "AutoFishing local snapshot fallback", fallbackSummary);
                    ResetAutoFishingMovementCancelCaseState();
                    terminal = true;
                    return FixtureAttemptResult.Succeeded;
                }
                throw new InvalidOperationException("The first-party primitive session was not activated after synthetic " + toggleButton + " keybind input. Is the official AutoFishing package enabled?");
            }
            catch (Exception ex)
            {
                ResetAutoFishingMovementCancelCaseState();
                terminal = true;
                runtime.Diagnostics.RecordError("DTMAPI.GameBridge", "Smoke auto-fishing movement-cancel exercise failed.", ex.ToString());
                runtime.SetHookStatus("Smoke.AutoFishingMovementCancel", "failed", "AutoFishing primitive snapshot + DTMAPI synthetic frame", ex.GetType().Name + ": " + ex.Message);
                return FixtureAttemptResult.Failed;
            }
            finally
            {
                if (syntheticMovementPressed)
                    DispatchQaInputFrame("A", isDownNow: false, pressedEdge: false, releasedEdge: true);
                if (terminal)
                    runtime.Input.ClearFrame();
            }
        ...
        private void ResetAutoFishingMovementCancelCaseState()
        {
            autoFishingMovementCancelReadyAt = DateTimeOffset.MinValue;
            autoFishingMovementCancelAwaitingNativeInput = false;
            autoFishingMovementCancelOwnerId = string.Empty;
            autoFishingHotkeyInjected = false;
        }
        ...
        private FixtureAttemptResult TryExerciseAutoFishingZeroPerformanceProfileForFixture(IAutoFishingPerformanceFixture performanceFixture, string profile, FishingRuntimeSession? session, string toggleButton, Type dolocApi)
        {
            ...
            if (autoFishingPerformanceAwaitingMovementCancel)
            {
                if (primitives.HasActiveSession)
                {
                    if (DateTimeOffset.UtcNow - autoFishingPerformanceMovementCancelRequestedAtUtc > TimeSpan.FromSeconds(30))
                        throw new TimeoutException("EnabledNoRod did not observe native HorizontalMoveFactor cancellation within 30 seconds of the external movement gate.");
                    return FixtureAttemptResult.Pending;
                }
                autoFishingPerformanceAwaitingMovementCancel = false;
                if (!primitives.LastReleaseReason.StartsWith("manual-move HorizontalMoveFactor=", StringComparison.Ordinal))
                    throw new InvalidOperationException("EnabledNoRod session ended without native HorizontalMoveFactor cancellation. releaseReason=" + primitives.LastReleaseReason + ".");
                performanceFixture.MarkMovementCancellation("native HorizontalMoveFactor");
                RestoreEnabledNoRodSelection(dolocApi, "native-movement-cancel");
                runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingMovementCancel OK owner=Yuuka.DTMAPI.AutoFishing, source=native HorizontalMoveFactor.");
                BeginPrimitiveAutoFishingPerformanceTitleCleanup(toggleButton, dolocApi, stopProduct: false);
                return FixtureAttemptResult.Pending;
            }
            ...
            if (profile.Equals("EnabledNoRod", StringComparison.OrdinalIgnoreCase))
            {
                ...
                runtime.RuntimeMonitor.Log("Smoke exercise AutoFishingMovementCancel awaiting native input owner=" + (session?.OwnerId ?? string.Empty) + ", profile=EnabledNoRod, source=Agent.Status.HorizontalMoveFactor.");
                runtime.SetHookStatus("Smoke.AutoFishingMovementCancel", "pending", "Agent.Status.HorizontalMoveFactor", "EnabledNoRod measurement complete; awaiting external physical A input.");
                return FixtureAttemptResult.Pending;
            }
        ...
        private FishingPerformanceTelemetrySnapshot CaptureFishingPerformanceTelemetry()
        {
            ...
            return new FishingPerformanceTelemetrySnapshot(
                ...
                native?.Current.HorizontalMoveFactorAvailable == true,
                visibleReel.Queued,
                visibleReel.Consumed,
                visibleReel.NativeAccepted,
                visibleReel.Retries,
        ...
        private AutoFishingPerformanceFixtureSnapshot CaptureAutoFishingPerformanceSnapshot()
        {
            ...
            return new AutoFishingPerformanceFixtureSnapshot
            {
                ...
                HorizontalMoveFactorAvailable = value.HorizontalMoveFactorAvailable,
        ...
        {
            internal FishingPerformanceTelemetrySnapshot(int accessorBuilds, int accessorRebuilds, int accessorBuildFailures, int accessorInvocationFailures, long casts, long fish, int legacyOptions, int legacyStates, int sessions, int inputLeases, int animationLeases, int hookRuntimes, int nativeTransient, int nativeReferences, bool selectedRod, bool horizontalMoveFactorAvailable, long visibleReelQueued, long visibleReelConsumed, long visibleReelNativeAccepted, long visibleReelRetries, long visibleReelTimeouts, long nativeTryCastEnergyGateUnavailable, long nativeTryCastInsufficientEnergy)
            {
            ...
            internal bool HorizontalMoveFactorAvailable { get; }
        ...
        private FixtureAttemptResult TryExerciseFirstPartyAutoFishingPhaseForFixture(FishingRuntimeSession session, string scenario, string toggleButton, Type dolocApi, IAutoFishingPerformanceFixture? performanceFixture)
        {
            ...
            progress.InstantBiteObserved |= diagnostics.NativeBitePreparedCount > 0;
            ...
            bool complete = progress.AutoCastObserved && progress.WaitObserved && progress.BiteReadyObserved &&
        ...
        private sealed class AutoFishingSmokeFlowProgress
        {
            ...
            internal void Reset(string scenario)
            {
                Scenario = scenario;
