...
namespace DTMAPI.GameBridge.DolocTown.QA
{
    internal sealed class AutoFishingPerformanceOrchestrator
    {
        ...
        private static void ApplyStart(FishingPerformanceResult result, AutoFishingPerformanceFixtureSnapshot value)
        {
            ...
            result.NativeMovementAvailableStart = value.NativeMovementAvailable;
        ...
        private static void ApplyEnd(FishingPerformanceResult result, AutoFishingPerformanceFixtureSnapshot value)
        {
            ...
            result.NativeMovementAvailableEnd = value.NativeMovementAvailable;
        ...
        private static void ValidateProfile(FishingPerformanceResult result)
        {
            ...
            else if (result.Profile.Equals("EnabledNoRod", StringComparison.OrdinalIgnoreCase) && (!result.NativeMovementAvailableStart || !result.NativeMovementAvailableEnd))
                failure = "EnabledNoRod did not retain current native movement availability at both measurement boundaries";
            if (failure.Length == 0)
                return;
