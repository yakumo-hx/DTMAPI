...
DTMAPI Auto Fishing is an admitted first-party Advanced CodeMod. It is optional; installing the five-assembly DTMAPI Runtime does not install or enable this product.

The product DLL owns its fishing state machine, cached native adapters, Harmony patch set, transactions, input/animation overrides and cleanup. It uses only the AutoFishing-specific SDK policy `doloctown-24456188-autofishing-v1`; it does not consume GameBridge, the deleted first-party primitive seam, or the frozen `IFishingAutomationApi`. That frozen public API remains a mandatory Runtime compatibility path for retained old binaries only.

Current boundary:

- DTMAPI-native `DtmMod` entry in canonical `Yuuka.DTMAPI.AutoFishing.dll`.
- SDK-generated `CodeModKind=Advanced` manifest, exact build-24456188 native reference receipt and canonical Harmony owner `dtmapi.mod.yuuka.dtmapi.autofishing`.
...
- Movement cancellation mirrors the current native Wait owner in order: exact nonzero `MoveModifier.inputMultiplier`, pre-`base.OnPlay` `abs(VelocityX) > 0.001f`, then exact nonzero `MoveModifier.OffsetX` after the base refresh. Missing or invalid native movement accessors stop automation; there is no hard-coded keyboard fallback or configurable epsilon.
- Optional behavior switches:
  - `Instant bite` skips the native waiting period after the hook reaches water, then reels into the normal minigame/result path. Its native roll is irreversible: required Wait fields are ordered with `waitForFishBite=false` as the final commit bit, while an uncertain commit closes automation and returns control to native Wait.
