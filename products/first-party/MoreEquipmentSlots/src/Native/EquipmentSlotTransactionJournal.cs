...
namespace DTMAPI.MoreEquipmentSlots
{
    ...
    {
        internal static string Compute(string? currentPath)
        {
            if (string.IsNullOrWhiteSpace(currentPath))
                return "native-v2|current=missing|prev=missing|bak=missing";

            string path = Path.GetFullPath(currentPath);
            var result = new StringBuilder("native-v2");
            Append(result, "current", path);
            Append(
                result,
                "prev",
                AddArchiveSuffix(path, "prev"));
            Append(
                result,
                "bak",
                AddArchiveSuffix(path, "bak"));
        ...
        private static string AddArchiveSuffix(
            string path,
            string suffix)
        {
            string directory =
