...
| Multi-platform structure or provenance | Run `tools/scripts/test-dtmapi-multiplatform-package.ps1 -PackageRoot <package>`. Add `-AllowWorkshopControlFile` only for the official local upload leaf described by the matrix; never for a repository candidate or Steam subscription. |
