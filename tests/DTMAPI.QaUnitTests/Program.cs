    ...
    internal static class Program
    {
        private static int Main()
        {
            ...
            try
            {
                ...
                Batch6AutoFishingManagerLifecycleRequiresThreeProcessFailClosedContract();
        ...
        private static void Batch6AutoFishingManagerLifecycleRequiresThreeProcessFailClosedContract()
        {
            string hash = new string('A', 64);
            string productRoot = Path.Combine(Path.GetTempPath(), Batch6AutoFishingPilotSettings.ProductUniqueId);
            var settings = new Batch6AutoFishingManagerLifecycleSettings
            {
                Enabled = true,
                Mode = "sameprocessdisable",
                ExpectedProductRoot = productRoot,
                ExpectedDisabledMarkerSha256 = hash,
                ExpectedPackageSha256 = hash,
                ExpectedEntryDllSha256 = hash,
                ExpectedManifestSha256 = hash,
                ExpectedReferencePolicySha256 = hash
            };
            settings.NormalizeAndValidate(caseSelected: true, saveSlot: 5);
            Assert(settings.IsSameProcessDisable && !settings.IsColdDisabled &&
                settings.DisabledMarkerPath == Path.Combine(Path.GetFullPath(productRoot), "dtmapi.disabled"),
                "Manager lifecycle settings must canonicalize the exact same-process marker contract on fifth save only.");
            settings.Mode = "ColdDisabled";
            settings.NormalizeAndValidate(caseSelected: true, saveSlot: 5);
            Assert(settings.IsColdDisabled,
                "Manager lifecycle settings must retain a distinct cold-disabled clean-process contract.");
            AssertThrows<InvalidDataException>(() => settings.NormalizeAndValidate(caseSelected: true, saveSlot: 3),
                "Manager lifecycle must reject any save other than the authoritative fifth save.");

            string root = FindRepositoryRoot();
            string coordinator = File.ReadAllText(Path.Combine(root, "products", "first-party", "AutoFishing", "qa", "batch6", "Batch6AutoFishingManagerLifecycleCoordinator.cs"));
            string participant = File.ReadAllText(Path.Combine(root, "src", "DTMAPI.GameBridge.DolocTown.QA", "QaHostParticipant.cs"));
            Assert(coordinator.Contains("requestProductOwnerRefresh", StringComparison.Ordinal) &&
                coordinator.Contains("QaScenarioController.RequestProductOwnerRefresh/ModManager.ReloadMods", StringComparison.Ordinal) &&
                coordinator.Contains("awaiting-create-disabled-marker", StringComparison.Ordinal) &&
                coordinator.Contains("awaiting-remove-disabled-marker", StringComparison.Ordinal) &&
                coordinator.Contains("RequireSameProcessRemoved", StringComparison.Ordinal) &&
                coordinator.Contains("RequireColdDisabled", StringComparison.Ordinal) &&
                coordinator.Contains("WorkshopReloadReceiptCount < 2", StringComparison.Ordinal) &&
                participant.Contains("scenarios.RequestProductOwnerRefresh", StringComparison.Ordinal) &&
                participant.Contains("batch6AutoFishingManagerLifecycle?.OnWorkshopReloadCompleted()", StringComparison.Ordinal),
                "Manager lifecycle must use two real QA-owned ModManager reload receipts and distinct same-process/cold-process fail-closed observations.");
        }
        ...
        private static void Batch6AutoFishingL5UsesQaOwnedTitleAndReentryLifecycle()
        {
            ...
            Assert(coordinatorSource.Contains("internal bool ShouldRequestReturnHome", StringComparison.Ordinal) &&
                ...
                coordinatorSource.Contains("waiting for the completed F6 release transaction", StringComparison.Ordinal) &&
                ...
                "L5 must expose coordinator-owned title/re-entry requests and delay ReturnHome until the F6 release transaction completes.");
        ...
        private static void QaParticipantOwnsHundredFishMeasurementAndTerminalResult()
        {
            ...
            AutoFishingPerformanceFixtureSnapshot Snapshot(long pullExited, int seconds) => new AutoFishingPerformanceFixtureSnapshot
            {
                ...
                NativeReferences = 2,
                SelectedRod = true,
                HorizontalMoveFactorAvailable = true,
        ...
        private static void G6LifecycleRoutingAndCallbackBoundaryAreClosed()
        {
            ...
            Assert(settingsSource.Contains("G6LifecycleCases", StringComparison.Ordinal) &&
                settingsSource.Contains("QA G6 save/load, pending-pressure, and long-title routes are mutually exclusive", StringComparison.Ordinal) &&
            ...
            Assert(participantSource.Contains("scenarios.PrepareG6Lifecycle(new G6FixtureOptions", StringComparison.Ordinal) &&
                ...
                participantSource.Contains("scenarios.NotifyG6SaveLoaded()", StringComparison.Ordinal) &&
            ...
            Assert(autoFishingSource.Contains("VerifyAutoFishingReportExportForFixture(scenario);", StringComparison.Ordinal) &&
                ...
                autoFishingSource.Contains("ResetAutoFishingMovementCancelCaseState();", StringComparison.Ordinal) &&
                autoFishingSource.Contains("autoFishingHotkeyInjected = false;", StringComparison.Ordinal) &&
                autoFishingSource.Contains("if (terminal)", StringComparison.Ordinal) &&
        ...
        private static void G5WorldMutationRoutingAndRestorationAreClosed()
        {
            ...
            Assert(runnerSource.Contains("ConfigDirectoryTransaction = 'forbidden'", StringComparison.Ordinal) &&
                ...
                "Runner must snapshot save/sidecar metadata before staging, forbid whole-config restore, leave the deleted settings channel untouched, and restore only explicit config files after stable process exit.");
        ...
        private sealed class FakeBatch6AutoFishingEntry
        {
            ...
            internal FakeBatch6AutoFishingEntry(bool enabled, bool toggleAwaitingRelease = false)
            {
                ...
                session = new FakeBatch6Session(isReleased: false);
