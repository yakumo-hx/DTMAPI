...
namespace DTMAPI.UnitTests
{
    internal static class MoreEquipmentSlotsProductTests
    {
        internal static void RunAll()
        {
            ...
            LegacySchema2AndHostSchema3MigrateWithoutItemLoss();
        ...
        private static void FixedThreeSlotAndEffectPolicy()
        {
            ...
            Assert(
                MoreEquipmentSlotsEffectPolicy
                    .ProductShieldMayHandleAttack(false, true) &&
                !MoreEquipmentSlotsEffectPolicy
                    .ProductShieldMayHandleAttack(true, true) &&
                MoreEquipmentSlotsEffectPolicy
                    .FindTailShieldIndex(slots) == 2 &&
                MoreEquipmentSlotsEffectPolicy
                    .PreservesNativeHatVisual,
                "Official shield priority, tail shield selection, and native hat appearance ownership regressed.");
        }
        ...
        private static void PlacementDistinguishesBackpackMailAndFailure()
        {
            ...
            Assert(
                ...
                "A successful native mail mutation followed by unreadable evidence must retain one outcome-unknown attempt.");
            unreadableAfterMail.ThrowMailReadAtCall = 0;
            NativePlacementResult reconciled =
                placement.ResolveUnknown(pending!);
            Assert(
                reconciled.Kind == NativePlacementKind.Mail &&
                unreadableAfterMail.MailSendCallCount == 1 &&
                unreadableAfterMail.MailCount == 1,
                "Later exact +1 mail evidence must reconcile without a second native mail call.");
            ...
            EquipmentSlotNativeMutationOutcomeUnknownException?
                unchangedPending = null;
            try
            {
                noMutationPlacement.PlaceOne("hat");
            }
            catch (
                EquipmentSlotNativeMutationOutcomeUnknownException
                    ex)
            ...
            unreadableWithoutMutation.ThrowMailReadAtCall = 0;
            NativePlacementResult unchanged =
                noMutationPlacement.ResolveUnknown(
                    unchangedPending!);
            Assert(
                ...
                "An unreadable post-call with an unchanged preimage must reconcile to one retained sidecar item and zero native mail.");
        }
        ...
        private static void AtomicInstallerOwnsRollbackAndDeactivationPaths()
        {
            ...
            Assert(
                ...
                runtime.Contains(
                    ...
                    StringComparison.Ordinal),
                "Atomic Hook installation must own exact-owner rollback, and real owner deactivation must detach callbacks and unpatch the exact ProductNative owner.");
        }
