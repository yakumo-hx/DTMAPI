...
internal static class Program
{
    ...
    private static void AdvancedIdentityAndReceiptFailClosedMatrix()
    {
        ...
        try
        {
            ...
            Assert(autoFishing.ProvenanceStatus == DoctorProvenanceStatus.VerifiedReferenceReceipt &&
                   ...
                   autoFishing.ReferencePolicyId == "doloctown-23762374-autofishing-v1" &&
            ...
            {
                value["referencePolicyId"] = "doloctown-23762374-autofishing-v1";
    ...
    private static void InstalledAdvancedCompatibilityUsesActualGameBytesAndSteamBuild()
    {
        ...
        try
        {
            ...
            DoctorArtifact incompatible = Artifact(
                new DoctorEngine().Inspect(gameRoot, new DoctorOptions { ScanContext = DoctorScanContext.InstalledGame }),
                "DTMAPI.AdvancedFixture");
            Assert(incompatible.ProvenanceStatus == DoctorProvenanceStatus.VerifiedReferenceReceipt &&
                   incompatible.ReferenceCompatibility == DoctorCompatibilityStatus.Incompatible &&
                   incompatible.GameCompatibility == DoctorCompatibilityStatus.Incompatible &&
                   !incompatible.Findings.Any(value => value.Code == "advanced-game-build-incompatible") &&
                   incompatible.Findings.Count(value => value.Code == "advanced-reference-hash-mismatch") == 2,
                "Installed-game Doctor must recompute both local reference files while reading only the top-level AppState buildid; a nested depot build cannot override the game build.");
            ...
            DoctorArtifact wrongBuild = Artifact(
                new DoctorEngine().Inspect(gameRoot, new DoctorOptions { ScanContext = DoctorScanContext.InstalledGame }),
                "DTMAPI.AdvancedFixture");
            Assert(wrongBuild.GameCompatibility == DoctorCompatibilityStatus.Incompatible &&
                   wrongBuild.Findings.Any(value => value.Code == "advanced-game-build-incompatible"),
                "A Steam build mismatch must be a structured blocking error independent of reference self-reporting.");

            File.Delete(Path.Combine(steamApps, "appmanifest_2285550.acf"));
            DoctorArtifact unknownBuild = Artifact(
                new DoctorEngine().Inspect(gameRoot, new DoctorOptions { ScanContext = DoctorScanContext.InstalledGame }),
                "DTMAPI.AdvancedFixture");
            Assert(unknownBuild.GameCompatibility == DoctorCompatibilityStatus.Unverifiable &&
                   unknownBuild.Findings.Any(value => value.Code == "advanced-game-build-unverifiable"),
                "An unavailable Steam build authority must fail closed as unverifiable, not be guessed from path or product identity.");
    ...
    private static void Draft202012SchemasValidateDoctorAndAdvancedContracts()
    {
        ...
        string autoFishingPolicyJson = File.ReadAllText(SchemaFixture("doloctown-23762374-autofishing-v1.json"), Encoding.UTF8);
    ...
    private const string AutoFishingPolicySha256 = "C434506848F53E21A6371C487B43941FB3408031479FA85E96BB77145AAD5557";
    private const string OneActionCompletePolicySha256 = "A44AF9101D19941B5F019D625A7E74D4D49CCE7CBCF7833E8B4ADCB6BED80A8D";

    private static AdvancedPackageFixture WriteAdvancedPackage(string root, string uniqueId, string manifestKind = "Advanced")
    {
