    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                PublicProductScanResult[] publicProducts = ScanRetainedPublicProducts(options);
        ...
        private static PublicProductScanResult[] ScanRetainedPublicProducts(IReadOnlyDictionary<string, string> options)
        {
            ...
            string expectedCandidateVersion)
        {
            ...
            for (int index = 0; index < paths.Length; index++)
            {
                ...
                ExternalConsumerMetadata metadata = ReadExternalConsumerMetadata(paths[index]);
                ...
                int resolvedMemberReferences = ResolveExternalConsumerMemberReferences(
                    paths[index],
                    candidatePath,
                    expectedCandidateVersion,
        ...
        private static ExternalConsumerMetadata ReadExternalConsumerMetadata(string assemblyPath)
        {
            using FileStream stream = File.OpenRead(assemblyPath);
            using var peReader = new PEReader(stream);
            if (!peReader.HasMetadata)
                throw new BadImageFormatException("Retained external consumer is not a managed assembly.", assemblyPath);
            ...
            return new ExternalConsumerMetadata(
        ...
        private static int ResolveExternalConsumerMemberReferences(
            string consumerPath,
            string candidatePath,
            string expectedCandidateVersion,
            IReadOnlyCollection<int> memberReferenceTokens)
        {
            ...
            try
            {
                ...
                RequireEqual(expectedCandidateVersion, candidate.GetName().Version?.ToString() ?? string.Empty, "external ABI candidate version");
                ...
                foreach (int token in memberReferenceTokens)
                {
                    MemberInfo member = consumer.ManifestModule.ResolveMember(token)
                        ?? throw new MissingMemberException("Unable to resolve retained external MemberRef 0x" + token.ToString("X8") + ".");
                    Assembly declaringAssembly = member.DeclaringType?.Assembly
                        ?? throw new MissingMemberException("Retained external MemberRef has no declaring assembly.");
                        ...
                        !string.Equals(Path.GetFullPath(declaringAssembly.Location), Path.GetFullPath(candidatePath), StringComparison.OrdinalIgnoreCase))
                    {
                        throw new MissingMemberException(
                            "Retained external MemberRef resolved outside the exact candidate DTMAPI.Abstractions assembly.");
                    }
                    resolved++;
                }
                return resolved;
        ...
        private sealed record ExternalConsumerMetadata(
            string AssemblyName,
