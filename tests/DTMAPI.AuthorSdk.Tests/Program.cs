...
internal static class Program
{
    ...
    public static async Task<int> Main()
    {
        ...
        string temporaryRoot = Path.Combine(repository, ".tmp", "author-sdk-tests-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            await TestAdvancedCodeModReferencePackageAndDeploy(temporaryRoot, compatibility, repository).ConfigureAwait(false);
            ...
            Console.WriteLine("DTMAPI Author SDK tests: OK");
            return 0;
        }
        finally
        {
            ...
            string expected = Path.GetFullPath(Path.Combine(repository, ".tmp")) + Path.DirectorySeparatorChar;
            if (Path.GetFullPath(temporaryRoot).StartsWith(expected, StringComparison.OrdinalIgnoreCase) && Directory.Exists(temporaryRoot))
    ...
    private static void TestPublishedDeploymentJournalContract(string repository)
    {
        ...
        True(
            readme.Contains("New deployment journals and every journal write use schema 3.", StringComparison.Ordinal),
            "Published README identifies deployment journal schema 3");
        True(
    ...
    private static async Task TestAdvancedCodeModReferencePackageAndDeploy(string temp, string compatibility, string repository)
    {
        ...
        string? installedGameRoot = FindInstalledGameRoot(repository);
        if (installedGameRoot == null)
        {
            Console.WriteLine("DTMAPI Author SDK Advanced fixture: SKIP (explicit local Doloc Town game root unavailable)");
            return;
        }

        string gameRoot = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-valid", "23762374");
...
""".Replace("\r\n", "\n", StringComparison.Ordinal), new UTF8Encoding(false));
        ...
        Equal("doloctown-23762374-autofishing-v1", autoFishingAuthor["advanced"]!["referencePolicyId"]!.GetValue<string>(),
...
""".Replace("\r\n", "\n", StringComparison.Ordinal), new UTF8Encoding(false));
        ...
        CommandReport autoFishingBuild = await ExpectSuccess(
            ...
            "--compatibility-root", compatibility,
            "--game-root", gameRoot).ConfigureAwait(false);
        Equal("doloctown-23762374-autofishing-v1", autoFishingBuild.Values["referencePolicyId"], "AutoFishing build policy identity");
        Equal("c434506848f53e21a6371c487b43941fb3408031479fa85e96bb77145aad5557", autoFishingBuild.Values["referencePolicySha256"], "AutoFishing build policy hash");
...
""".Replace("\r\n", "\n", StringComparison.Ordinal), new UTF8Encoding(false));
        ...
        string tamperGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-tamper", "23762374");
        ...
        string duplicateRootGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-duplicate-root", "23762374");
        ...
        string unknownRootGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-unknown-root", "23762374");
        ...
        string missingRootGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-missing-root", "23762374");
        ...
        string duplicateNestedGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-duplicate-nested", "23762374");
        ...
        string unknownNestedGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-unknown-nested", "23762374");
        ...
        string missingNestedGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-missing-nested", "23762374");
        ...
        string missingReceiptGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-missing-receipt", "23762374");
        ...
        string bundledGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-bundled-native", "23762374");
        ...
        string bundledHelperGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-bundled-helper", "23762374");
        ...
        string wrongBuildGame = CreateAdvancedGameFixture(temp, installedGameRoot, "advanced-wrong-build", "1");
    ...
    private static async Task TestValidationFailures(string temp, string compatibility)
    {
        ...
        string future = Path.Combine(temp, "invalid-future-runtime");
        ...
        manifest["MinimumDTMApiVersion"] = "0.6.0";
        WriteJson(Path.Combine(future, "manifest.json"), manifest);
        HasCode(await ExpectFailure("future runtime", "validate", future).ConfigureAwait(false), "SDK105", "Runtime 0.5.5-only gate");
    ...
    private static string CreateAdvancedGameFixture(string temp, string installedGameRoot, string label, string buildId)
    {
        ...
        {
            string source = Path.Combine(installedGameRoot, relative.Replace('/', Path.DirectorySeparatorChar));
    ...
    private static string CreateCompatibilityPayload(string repository, string temp)
    {
        ...
        {
            string staged = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(selfContainedExecutable))!, "compatibility", "0.5.5");
