...
namespace DTMAPI.UnitTests
{
    internal static class MoreEquipmentSlotsProductTests
    {
        internal static void RunAll()
        {
            ...
            ItemAdmissionPreservesZeroThreeOneSemantics();
            DynamicNativeTailLayoutPreservesFullSizeSlots();
            ...
            NewGameResetDeletesOnlyExactProductSlotDirectory();
        ...
        private static void
            ItemAdmissionPreservesZeroThreeOneSemantics()
        {
            MoreEquipmentSlotsItemAdmission passive =
                MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    isNativePassiveItem: true,
                    isNativeHatItem: false,
                    "DolocTown.Config.Item.ItemFunctionPassive",
                    "skill-a",
                    isShieldHat: false,
                    maxShieldValue: 0);
            MoreEquipmentSlotsItemAdmission herb =
                MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    isNativePassiveItem: true,
                    isNativeHatItem: false,
            ...
            Assert(
                ...
                herb.Kind ==
                    MoreEquipmentSlotsItemKind.PassiveEquipment,
                "Both historical passive-equipment function families with a native skill must remain accepted.");

            Assert(
                !MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    true,
                    false,
                    "DolocTown.Config.Item.ItemFunctionCooldown",
                    "skill-c",
                    false,
                    0).Accepted &&
                !MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    ...
                    false,
                    0).Accepted,
                "Active/other passive subclasses and skill-less passive equipment must be rejected.");

            MoreEquipmentSlotsItemAdmission ordinaryHat =
                MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    false,
                    true,
                    "DolocTown.Config.Item.ItemFunctionHat",
                    string.Empty,
                    false,
                    0);
            MoreEquipmentSlotsItemAdmission shieldHat =
                MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    false,
                    true,
                    "DolocTown.Config.Item.ItemFunctionHatShield",
                    "shield",
                    true,
                    10);
            Assert(
                ...
                !MoreEquipmentSlotsItemAdmissionPolicy.Decide(
                    false,
                    false,
                    "DolocTown.Config.Item.ItemFunctionActive",
                    "active",
                    false,
                    0).Accepted,
                "Registered ordinary hats may have zero defense/empty skill, shield hats require positive MaxShieldValue, and active/other items remain rejected.");
        }

        private static void
            DynamicNativeTailLayoutPreservesFullSizeSlots()
        {
            MoreEquipmentSlotsUiLayout maximumReviewed =
                MoreEquipmentSlotsUiLayoutPolicy.Calculate(
                    officialSlotWidth: 112f,
                    officialSlotHeight: 112f,
                    observedSpacing: 12f,
                    officialPassiveCount: 5);
            bool everyReviewedCountIsValid = true;
            for (int officialPassiveCount = 1;
                 officialPassiveCount <= 5;
                 officialPassiveCount++)
            {
                everyReviewedCountIsValid &=
                    MoreEquipmentSlotsUiLayoutPolicy.Calculate(
                        112f,
                        112f,
                        12f,
                        officialPassiveCount).Valid;
            }
            Assert(
                everyReviewedCountIsValid &&
                maximumReviewed.Valid &&
                MoreEquipmentSlotsUiLayoutPolicy.SlotSize ==
                    112f &&
                maximumReviewed.ContentWidth == 360f &&
                MoreEquipmentSlotsUiLayoutPolicy.ProductSlotX(
                    maximumReviewed,
                    0) == 56f &&
                MoreEquipmentSlotsUiLayoutPolicy.ProductSlotX(
                    maximumReviewed,
                ...
                MoreEquipmentSlotsUiLayoutPolicy.ProductSlotX(
                    maximumReviewed,
                    2) == 304f,
                "The Product row must retain three unscaled 112x112 slots at native 12px spacing for every reviewed official passive count from one through five.");
            Assert(
                !MoreEquipmentSlotsUiLayoutPolicy.Calculate(
                    112f,
                    112f,
                ...
                !MoreEquipmentSlotsUiLayoutPolicy.Calculate(
                    ...
                    10f,
                    5).Valid,
                "An unreviewed official count, slot size, or spacing must block Product UI instead of guessing, shrinking, or wrapping.");
        }
        ...
        private static void HookOwnershipRejectsBothOrdersAndResidue()
        {
            ...
            Assert(
                ...
                "Real owner deactivation must observe five exact-owner hooks removed.");
        }
        ...
        private static void PlacementDistinguishesBackpackMailAndFailure()
        {
            ...
            Assert(
                ...
                "A true withdrawal Boolean without the exact -1 count delta must become outcome-unknown.");

            var mutateThenThrow = new FakeGateway
            {
                CostThrowAfterMutation = true
            };
            mutateThenThrow.SetBackpackCount(1);
            bool mutateThenThrowUnknown = false;
            try
            {
                new EquipmentSlotNativePlacement(
                    mutateThenThrow)
                ...
                EquipmentSlotNativeMutationOutcomeUnknownException)
            {
                mutateThenThrowUnknown = true;
            }
            Assert(
                mutateThenThrowUnknown &&
                mutateThenThrow.BackpackCount == 0,
                "A mutate-then-throw CostItem path must retain its exact native mutation as outcome-unknown and must not replay.");

            var unreadableAfterWithdrawal = new FakeGateway
            {
                ThrowBackpackReadAtCall = 2
            };
            unreadableAfterWithdrawal.SetBackpackCount(1);
            bool unreadableAfterUnknown = false;
            try
            {
                new EquipmentSlotNativePlacement(
                    unreadableAfterWithdrawal)
                    .TryWithdrawOne("hat");
            }
            catch (
                EquipmentSlotNativeMutationOutcomeUnknownException)
            {
                unreadableAfterUnknown = true;
            }
            Assert(
                unreadableAfterUnknown &&
                unreadableAfterWithdrawal.BackpackCount == 0,
                "An unreadable post-withdrawal count must become outcome-unknown after one mutation.");

            var unchangedFailure = new FakeGateway
            {
                CostReportedOverride = false,
                CostDelta = 0
            };
            unchangedFailure.SetBackpackCount(1);
            Assert(
                !new EquipmentSlotNativePlacement(
                    unchangedFailure)
                    .TryWithdrawOne("hat") &&
                unchangedFailure.BackpackCount == 1,
                "Only an explicit false return plus exact unchanged evidence is an ordinary retryable withdrawal failure.");
        }
            ...
            IdenticalNativeBytesStillProveRewrite()
        {
            ...
            Assert(
                ...
                !string.Equals(
                    ...
                    "missing",
                    StringComparison.Ordinal),
                "The explicit NewGame preimage must allow only current=missing while retaining real prev and bak evidence.");
            File.WriteAllBytes(currentPath, bytes);
            Assert(
                string.Equals(
                    EquipmentSlotNativeCommitFingerprint
                    ...
                    EquipmentSlotNativeCommitFingerprint
                        .Compute(
                            ...
                            backupCount: 5),
                    StringComparison.Ordinal),
                "The NewGame fingerprint route must converge to the strict fingerprint as soon as current exists.");
        ...
        private static void
            NewGameResetDeletesOnlyExactProductSlotDirectory()
        {
            string root = CreateMigrationRoot(
                "newgame-slot-reset");
            ...
            File.WriteAllText(sidecarPath, "live");
            File.WriteAllText(
            ...
            bool deletedAgain =
                store.DeleteSlotDirectoryForNewGame(
                    sidecarPath,
                    archiveIndex: 11);
            bool mismatchedSlotRejected = false;
            try
            {
                store.DeleteSlotDirectoryForNewGame(
                    siblingPath,
            ...
            {
                mismatchedSlotRejected = true;
            }
            var newScope = new EquipmentSlotSaveScope
            {
                ArchiveIndex = 11,
                PlayerName = string.Empty,
                CustomPlayerName = string.Empty,
                TotalGameSeconds = 0
            };
            ...
            Assert(
                ...
                mismatchedSlotRejected &&
                empty.Scope.ArchiveIndex == 11 &&
                empty.Slots.Count == 3 &&
                !empty.Slots.Exists(slot => slot.IsOccupied),
                "NewGame reset must delete the complete exact Product slot directory idempotently, preserve siblings, reject slot drift, and create three empty in-memory slots.");
        }
        ...
        private static void AtomicInstallerOwnsRollbackAndDeactivationPaths()
        {
            ...
            Assert(
                ...
                installer.Contains(
                    ...
                    StringComparison.Ordinal) &&
                installer.Contains(
                    "ResolveRenderPassiveItems()",
                    StringComparison.Ordinal) &&
                installer.Contains(
                    "ResolveAccessoriesBarSelectablesGetter()",
                    StringComparison.Ordinal) &&
                installer.Contains(
                    "ResolveAccessoriesBarMethod(\"ClearCallBack\")",
                    StringComparison.Ordinal) &&
                !installer.Contains(
                    "ResolveAccessoriesBarMethod(\"__Init\")",
                    StringComparison.Ordinal) &&
                !installer.Contains(
                    "ResolveAccessoriesBarMethod(\"OnStartShow\")",
                    StringComparison.Ordinal) &&
                installer.Contains(
                ...
                runtime.Contains(
                    ...
                    StringComparison.Ordinal) &&
                !runtime.Contains(
                    "DTMAPI.MoreEquipmentSlots.Toggle",
                    StringComparison.Ordinal) &&
                !runtime.Contains(
                    "DTMAPI.MoreEquipmentSlots.Drawer",
                    StringComparison.Ordinal) &&
                !runtime.Contains(
                    "DisableDroneInteraction",
                    StringComparison.Ordinal) &&
                !runtime.Contains(
                    " void Update(",
                    StringComparison.Ordinal) &&
                ...
                !runtime.Contains(
                    ...
                    StringComparison.Ordinal),
                "Atomic Hook installation must own exact-owner rollback, provide the product shield only through the native shield-provider boundary, keep UI layout event-driven without a per-frame Product loop, and leave the official attack tail unreplicated.");
        }
            ...
            INativeItemPlacementGateway
        {
            ...
            internal int ThrowBackpackReadAtCall { get; set; }

            internal bool CostThrowAfterMutation { get; set; }

            internal int BackpackReadCallCount { get; private set; }
            ...
            {
                BackpackReadCallCount++;
                if (ThrowBackpackReadAtCall > 0 &&
                    BackpackReadCallCount ==
                        ThrowBackpackReadAtCall)
                {
                    throw new InvalidDataException(
                        "Injected unreadable native backpack authority.");
                }
            ...
            public bool TryCostBackpack(string itemId)
            {
                ...
                if (CostThrowAfterMutation)
                {
                    throw new InvalidOperationException(
                        "Injected CostItem mutate-then-throw failure.");
                }
