...
"%ComSpec%" /d /e:on /v:off /c call "%DTMAPI_DISPATCHER%" full-uninstall
set "DTMAPI_EXIT=%ERRORLEVEL%"
exit /b %DTMAPI_EXIT%

:dtmapi_missing
echo [ERROR] DTMAPI complete-uninstall files are missing.
echo [HELP] Copy the whole DTMAPI folder to a path containing only English letters and numbers, then try again.
echo [HELP] If files are still missing, resubscribe to DTMAPI.
