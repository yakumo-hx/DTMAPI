...
setlocal
set "SCRIPT_DIR=%~dp0"
set "DTMAPI_PS_PROBE_LIST=%SCRIPT_DIR%Content\DTMAPIInstaller\tools\common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\release-common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\install-to-game.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\install-bepinex.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\probe-powershell-host.ps1"
...
echo [INFO] Running installer script: %SCRIPT_DIR%Content\DTMAPIInstaller\tools\install-to-game.ps1
"%DTMAPI_POWERSHELL%" -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Content\DTMAPIInstaller\tools\install-to-game.ps1" -InstallBepInEx
set "DTMAPI_EXIT=%ERRORLEVEL%"
if not "%DTMAPI_EXIT%"=="0" (
  echo.
  echo [ERROR] DTMAPI install failed.
  echo [ERROR] Host used: %DTMAPI_POWERSHELL%
  echo [ERROR] Exit code: %DTMAPI_EXIT%
  echo [INFO] Run 3_check_dtmapi_status.bat and include the whole output when asking for help.
  echo [INFO] If an install-state.failed-*.json path was printed above, include that file too.
  pause
  exit /b %DTMAPI_EXIT%
)
echo.
echo [OK] DTMAPI install finished.
echo [OK] Host used: %DTMAPI_POWERSHELL%
echo [INFO] Run 3_check_dtmapi_status.bat to verify installed files.
pause
exit /b 0
