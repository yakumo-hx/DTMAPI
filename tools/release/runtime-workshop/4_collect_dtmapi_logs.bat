...
setlocal
set "SCRIPT_DIR=%~dp0"
set "DTMAPI_PS_PROBE_LIST=%SCRIPT_DIR%Content\DTMAPIInstaller\tools\common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\release-common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\collect-logs.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\analyze-startup-evidence.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\probe-powershell-host.ps1"
...
echo [INFO] Running log collector: %SCRIPT_DIR%Content\DTMAPIInstaller\tools\collect-logs.ps1
"%DTMAPI_POWERSHELL%" -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Content\DTMAPIInstaller\tools\collect-logs.ps1" -CaseId PLAYER-CRASH -DesktopTimestampOutput
set "DTMAPI_EXIT=%ERRORLEVEL%"
if not "%DTMAPI_EXIT%"=="0" (
  echo.
  echo [ERROR] Failed to collect DTMAPI logs.
...
echo.
echo [OK] DTMAPI logs collected.
echo [OK] Host used: %DTMAPI_POWERSHELL%
echo The output path was printed above.
