...
setlocal
set "SCRIPT_DIR=%~dp0"
set "DTMAPI_PS_PROBE_LIST=%SCRIPT_DIR%Content\DTMAPIInstaller\tools\common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\release-common.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\uninstall-dtmapi.ps1;%SCRIPT_DIR%Content\DTMAPIInstaller\tools\probe-powershell-host.ps1"
...
echo [INFO] Running uninstall script: %SCRIPT_DIR%Content\DTMAPIInstaller\tools\uninstall-dtmapi.ps1
"%DTMAPI_POWERSHELL%" -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Content\DTMAPIInstaller\tools\uninstall-dtmapi.ps1"
set "DTMAPI_EXIT=%ERRORLEVEL%"
if not "%DTMAPI_EXIT%"=="0" (
  echo.
  echo [ERROR] DTMAPI uninstall failed.
...
echo.
echo [OK] DTMAPI uninstall finished.
