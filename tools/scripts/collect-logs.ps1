...
$script:DtmUnityCrashMaxFileBytes = 96MB
$script:DtmUnityCrashMaxDumpFileBytes = 384MB
$script:DtmUnityCrashMaxTotalBytes = 128MB
...
if ($DesktopTimestampOutput) {
    ...
    $outputStamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $OutputDirectory = Join-Path (Join-Path $desktop 'DTMAPI-logs') $outputStamp
}
if ($OutputDirectory) {
    try {
        ...
        $failedDesktopOutput = $OutputDirectory
        $fallbackRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'DTMAPI-logs'
        $OutputDirectory = Join-Path $fallbackRoot $outputStamp
        Add-DtmCollectWarning -Message "Desktop log output '$failedDesktopOutput' could not be created: $($_.Exception.Message). Falling back to '$OutputDirectory'."
        New-Item -ItemType Directory -Force -Path $OutputDirectory | Out-Null
        $evidence = (Resolve-Path -LiteralPath $OutputDirectory).Path
    }
}
...
elseif (-not $gameDir) {
    ...
    Add-DtmCollectWarning -Message 'Player Doctor was skipped because the game folder could not be resolved.'
}
else {
    $playerDoctorStatus = 'missing'
    Add-DtmCollectWarning -Message "Player Doctor helper is missing: $playerDoctorExe"
}
...
if (-not [string]::IsNullOrWhiteSpace($dtmapiDir)) {
    $paths += @{ Path = Join-Path $dtmapiDir 'logs\latest.log'; Name = 'DTMAPI-latest.log' }
...
if (-not [string]::IsNullOrWhiteSpace($gameDir)) {
    $paths += @{ Path = Join-Path $gameDir 'BepInEx\LogOutput.log'; Name = 'BepInEx-LogOutput.log' }
}
...
Copy-DtmNewestEvidenceFileIfExists -Paths @($unityLogRoots | ForEach-Object { Join-Path $_ 'Player.log' }) -Destination (Join-Path $evidence 'Unity-Player.log')
Copy-DtmNewestEvidenceFileIfExists -Paths @($unityLogRoots | ForEach-Object { Join-Path $_ 'Player-prev.log' }) -Destination (Join-Path $evidence 'Unity-Player-prev.log')
...
if (-not [string]::IsNullOrWhiteSpace($dtmapiLogDir) -and (Test-Path -LiteralPath $dtmapiLogDir)) {
    ...
    if ($historyLogs.Count -gt 0) {
        ...
        foreach ($log in $historyLogs) {
            Copy-DtmEvidenceFileIfExists -Path $log.FullName -Destination (Join-Path $historyDir $log.Name)
        }
    }
}
