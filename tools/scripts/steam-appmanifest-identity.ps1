...
function Get-DtmApiSteamBranchIdentity {
    ...
    $pendingBranchSwitch = -not $userBranch.Equals($mountedBranch, [System.StringComparison]::OrdinalIgnoreCase)
    if ($pendingBranchSwitch -and -not $AllowPendingBranchSwitch) {
        throw "Steam manifest branch conflict: UserConfig='$userBranch', MountedConfig='$mountedBranch'."
    }
    if ($pendingBranchSwitch -and [string]::IsNullOrWhiteSpace($ExplicitBranch)) {
        throw "A pending Steam branch switch requires both -AllowPendingBranchSwitch and an explicit -Branch matching MountedConfig ('$mountedBranch')."
    }

    # MountedConfig identifies the branch whose depot bytes are currently installed. During a
    # pending branch switch UserConfig is only the requested destination, so it must not relabel
    # the bytes being frozen.
    $detectedBranch = $(if ($pendingBranchSwitch) { $mountedBranch } else { $userBranch }).ToLowerInvariant()
    ...
    return [pscustomobject]@{
        ...
        Source = $(if ($pendingBranchSwitch) { 'appmanifest.MountedConfig(pending UserConfig mismatch)' } else { 'appmanifest.UserConfig+MountedConfig' })
        IsKnownBranch = $isKnownBranch
        UserBranch = $userBranch.ToLowerInvariant()
        MountedBranch = $mountedBranch.ToLowerInvariant()
...
function Get-DtmApiSteamBuildIdentity {
    ...
    if ($AllowPendingBranchSwitch) {
        $branchParameters['AllowPendingBranchSwitch'] = $true
    }
