...
function Assert-ReleaseContractExactArtifactRoot {
    ...
    if ([string]::IsNullOrWhiteSpace($Root)) {
        Add-ReleaseContractFailure "$Label requires one explicit artifact root."
        return
    }
    try {
        ...
        Add-ReleaseContractFailure ("{0} is invalid: {1}" -f $Label, $_.Exception.Message)
        return
    }
    if (-not (Test-Path -LiteralPath $resolvedRoot -PathType Container)) {
        Add-ReleaseContractFailure "$Label is missing: $resolvedRoot"
        return
    }

    $actualCatalogIds = @(Get-ChildItem -LiteralPath $resolvedRoot -Directory -Force | ForEach-Object { $_.Name } | Sort-Object)
...
$productMigrationTargetMinimum = '0.5.5'
$authorSdkApiTarget = '0.5.5'
Assert-ReleaseContractEqual -Label 'Public product count' -Actual $publicProducts.Count -Expected 11

try {
    $releaseArtifactProducts = @(Get-DtmApiReleaseContractAdvancedProducts -Catalog $catalog)
}
catch {
    Add-ReleaseContractFailure ("0.6 exact Advanced artifact selection failed: {0}" -f $_.Exception.Message)
    $releaseArtifactProducts = @()
}
$releaseArtifactCatalogIds = @($releaseArtifactProducts | ForEach-Object {
    [string](Get-DtmApiMapValue -Map $_ -Key 'catalogId' -Default '')
})
$releaseArtifactIdSet = @{}
...
foreach ($product in $publicProducts) {
    ...
    $isReleaseArtifact = $releaseArtifactIdSet.ContainsKey($catalogId)
    ...
    if ($null -ne $manifest) {
        Assert-ReleaseContractEqual -Label "$catalogId publish manifest name" -Actual (Get-ReleaseContractValue $publishRow 'manifestName') -Expected (Get-ReleaseContractValue $manifest 'Name')
    }
    if ($null -ne $officialInfo) {
        ...
        Assert-ReleaseContractEqual -Label "$catalogId publish author" -Actual (Get-ReleaseContractValue $publishRow 'author') -Expected (Get-ReleaseContractValue $officialInfo 'author')
        Assert-ReleaseContractEqual -Label "$catalogId publish mod name" -Actual (Get-ReleaseContractValue $publishRow 'modName') -Expected (Get-ReleaseContractValue $officialInfo 'name')
        Assert-ReleaseContractEqual -Label "$catalogId publish game description" -Actual (Get-ReleaseContractValue $publishRow 'gameDescription') -Expected (Get-ReleaseContractValue $officialInfo 'description')
        Assert-ReleaseContractTrue -Label "$catalogId publish Steam name is required." -Condition (-not [string]::IsNullOrWhiteSpace([string](Get-ReleaseContractValue $publishRow 'steamName')))
        Assert-ReleaseContractTrue -Label "$catalogId publish Steam description is required." -Condition (-not [string]::IsNullOrWhiteSpace([string](Get-ReleaseContractValue $publishRow 'steamDescription')))
        if ($catalogId -eq 'more-saves') {
            $expectedMoreSavesTags = @('Mod', 'Utility', 'Functional', 'DTMAPI', 'Save', 'Chinese')
            $officialMoreSavesTags = @((Get-ReleaseContractValue $officialInfo 'tags') | ForEach-Object { [string]$_ })
            Assert-ReleaseContractEqual -Label 'more-saves official-info exact tags' -Actual ($officialMoreSavesTags -join '|') -Expected ($expectedMoreSavesTags -join '|')
            $moreSavesAuthor = Read-ReleaseContractJson -Path (Join-Path $repo ($sourceRoot + '/dtmapi.author.json').Replace('/', '\'))
            if ($null -ne $moreSavesAuthor) {
                $authorPublish = Get-ReleaseContractValue $moreSavesAuthor 'publish'
                $authorMoreSavesTags = @((Get-ReleaseContractValue $authorPublish 'tags') | ForEach-Object { [string]$_ })
                Assert-ReleaseContractEqual -Label 'more-saves Author SDK publish exact tags' -Actual ($authorMoreSavesTags -join '|') -Expected ($expectedMoreSavesTags -join '|')
            }
        }
    }
    ...
    if ($authorSdkProject) {
        ...
        elseif ($catalogId -ceq 'more-equipment-slots') {
            Assert-ReleaseContractEqual -Label 'MoreEquipment retained published version' -Actual (Get-ReleaseContractValue $product 'publishedVersion') -Expected '0.3.1-dtmapi'
            foreach ($artifactCase in @(
                [pscustomobject]@{ Label = 'primary'; Path = $primaryAuthorSdkRoot },
                [pscustomobject]@{ Label = 'repeat'; Path = $repeatAuthorSdkRoot }
            )) {
                if (-not [string]::IsNullOrWhiteSpace([string]$artifactCase.Path)) {
                    Assert-ReleaseContractTrue `
                        -Label "MoreEquipment $($artifactCase.Label) 1.0 Author SDK artifact must remain absent while Branch B is unresolved." `
                        -Condition (-not (Test-Path -LiteralPath ([string]$artifactCase.Path)))
                }
            }
        }
    }
