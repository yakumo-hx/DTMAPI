...
if ($resolveCount -ne 11) { Add-Failure "ActionSpeed Hook inventory must resolve exactly eleven targets; found $resolveCount." }
...
if (-not $installer.Contains('Resolve(gameAssembly, "DolocTown.AgentStateWater", "OnEnter", 0') -or
    -not $installer.Contains('Resolve(gameAssembly, "DolocTown.AgentStateWater", "OnExit", 0')) {
    Add-Failure 'ActionSpeed must own the independent manual-watering enter/exit lifecycle.'
}
...
foreach ($token in @('originalAnimatorSpeeds', 'pendingNativeAnimalInteract', 'UseItemContinues', 'InteractContinues', 'ItemBottle', 'PlantBasin', 'ResinCollector', 'ReadMember(state, "waterCan")', 'IsTypeOrBase(type, "DolocTown.ItemWaterCan")', 'AgentStateWater.OnExit', 'TryWriteAnimatorSpeed')) {
...
foreach ($token in @(
    ...
    "'Local.' + [string]`$_.officialFolder",
    'function Set-SmokeRecoveryOnlyAuthorSourceState',
    "Purpose = 'RecoveryOnlyClear'",
    'Functional smoke profiles never publish LocalDevelopment source')) {
    Require-Token 'Official Local/Workshop smoke source selection' $smokeRunner $token
}
foreach ($forbidden in @(
    "`$product.PSObject.Properties['codeModKind']",
    "Join-Path (Join-Path `$canonicalGameRoot 'Mods')",
    '.dtmapi-author-receipt.json',
    "mode = 'LocalDevelopment'")) {
    if ($smokeRunner.Contains($forbidden)) {
        Add-Failure "Smoke source selection still admits the retired <game>/Mods Author deployment lane: $forbidden"
    }
}
...
foreach ($file in 'AgentStateTool.cs','AgentStateWater.cs','ItemWaterCan.cs','AgentStateInteract.cs','AgentStateEat.cs','AgentControllerState.cs','AnimalRenderer.cs','PlantBasin.cs','ResinCollector.cs') {
...
Write-Host ("DTMAPI Batch 6 ActionSpeed Advanced product checks: OK (source-files={0}, hooks=11, policies=1, native-authorities=9)" -f $productFiles.Count)
