...
function Invoke-TransactionInstaller {
    ...
    try {
        ...
        if ($AllDeveloperDefinitions) {
            $arguments += @('-InstallAllDevOfficialMods', '-LegacyOfficialLocalOnly')
        }
        else {
            $arguments += @('-InstallPublishedModsOnly', '-LegacyOfficialLocalOnly')
        }
...
try {
    ...
    foreach ($requiredAuthorSdkTransactionToken in @(
        ...
        '& $authorSdkExe install-local-status ([string]$Mod.UniqueID)',
        'Mutation was not replayed',
        '--expected-package-sha256',
        "Source = 'AuthorSdkAtomicLocalInstall'",
        '[string]$catalogRows[0].sourceVersion',
        'because its exact Author SDK deployment receipt owns the destination',
        "DestructiveAuthority = 'ExactAuthorSdkDeploymentReceiptOnly'")) {
        Assert-InstallTransactionTest -Condition ($installText.Contains($requiredAuthorSdkTransactionToken)) `
            -Message "Managed Author SDK installer transaction is missing: $requiredAuthorSdkTransactionToken"
    }
    foreach ($forbiddenOuterOwnerToken in @(
        ...
        Assert-InstallTransactionTest -Condition (-not $installText.Contains($forbiddenOuterOwnerToken)) `
            -Message "Installer still contains the retired outer Author SDK transaction owner: $forbiddenOuterOwnerToken"
    }
    ...
    Assert-InstallTransactionTest -Condition ($installText -match 'Author SDK package/deployment authority is unavailable') -Message 'Installer no longer keeps managed Advanced deployment fail-closed.'
