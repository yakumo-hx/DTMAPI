...
try {
    ...
    foreach ($hook in @('awaiting-initial-enable-f6','awaiting-disable-f6','l4-qa-recovery-active','awaiting-title','awaiting-fifth-save-reentry','awaiting-reentry-enable-f6','awaiting-final-disable-f6','Smoke.Batch6.AutoFishingPilot = verified.','Smoke.Batch6.AutoFishingPilot.Cleanup = verified.')) {
    ...
    Assert-True ($smokeText.Contains('ForegroundMatchedAtSend') -and $smokeText.Contains('SendInputSucceeded') -and
        $smokeText.Contains('PostMessageFallbackUsed') -and $smokeText.Contains('batch6-autofishing-runner-input.json')) 'Batch 6 F6 inputs must bind foreground/SendInput/no-fallback provenance into evidence.'
    ...
    Assert-True ($reflectionObserverText.Contains('ToggleAwaitingRelease = ReadBooleanField(entry, "toggleAwaitingRelease")') -and
        ...
        $pilotCoordinatorText.Contains('completed F6 release transaction before QA requests ReturnHome')) `
        'L5 must not insert ReturnHome into an unfinished physical F6 disable transaction.'
    ...
    $policyHash = 'B' * 64
    $referenceBytes = $utf8.GetBytes((([ordered]@{ uniqueId='Yuuka.DTMAPI.AutoFishing'; referencePolicyId='doloctown-23762374-autofishing-v1'; referencePolicySha256=$policyHash; harmonyOwner='dtmapi.mod.yuuka.dtmapi.autofishing' } | ConvertTo-Json -Depth 5) + "`n"))
    ...
    $summary = [ordered]@{ schemaVersion=1; status='Passed'; uniqueId='Yuuka.DTMAPI.AutoFishing'; version='0.1.0'; codeModKind='Advanced'; policyId='doloctown-23762374-autofishing-v1'; harmonyOwner='dtmapi.mod.yuuka.dtmapi.autofishing'; gameBuildId='23762374'; packagePath=$packagePath; packageSha256=$packageHash; entryDllSha256=$entryHash; advancedReferenceReceiptSha256=$referenceHash }
