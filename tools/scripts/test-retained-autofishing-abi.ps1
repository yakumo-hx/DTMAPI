...
if (-not [string]::IsNullOrWhiteSpace($RetainedPublicWorkshopRoot)) {
    ...
    if ($retainedAuditProducts.Count -ne 11) {
        throw "Expected exactly 11 retained public products in the binary audit, found $($retainedAuditProducts.Count)."
    }
    $retainedAuditByProductId = @{}
    foreach ($auditProduct in $retainedAuditProducts) {
        $auditProductId = [string]$auditProduct.productId
        if ([string]::IsNullOrWhiteSpace($auditProductId) -or $retainedAuditByProductId.ContainsKey($auditProductId)) {
            throw "Retained binary audit contains a missing or duplicate productId: '$auditProductId'."
        }
        $retainedAuditByProductId[$auditProductId] = $auditProduct
    }
    foreach ($product in $publicProducts) {
        $catalogId = [string]$product.catalogId
        if (-not $retainedAuditByProductId.ContainsKey($catalogId)) {
            throw "Retained binary audit is missing Catalog public product '$catalogId'."
        }
        ...
        $auditProduct = $retainedAuditByProductId[$catalogId]
        if ([string]$auditProduct.fileName -cne $matches[0].Name) {
            throw "Retained product '$catalogId' file name drifted. Expected '$([string]$auditProduct.fileName)', found '$($matches[0].Name)'."
        }
        $actualProductSha256 = (Get-FileHash -LiteralPath $matches[0].FullName -Algorithm SHA256).Hash.ToUpperInvariant()
        $expectedProductSha256 = ([string]$auditProduct.sha256).ToUpperInvariant()
        if ($actualProductSha256 -cne $expectedProductSha256) {
            throw "Retained product '$catalogId' SHA-256 drifted. Expected '$expectedProductSha256', found '$actualProductSha256'."
        }
        $retainedProductDlls += $matches[0].FullName
        $retainedProductIds += $catalogId
    }
...
Write-Host "RetainedPublicProductExactHashCount=$($retainedProductDlls.Count)"
Write-Host "RetainedExternalConsumerInputCount=$($retainedExternalDlls.Count)"
