...
$unicodeReadOnly = -join @([char]0x53EA, [char]0x8BFB)
$unicodeContains = [char]0x542B
$unicodeGame = -join @([char]0x6E38, [char]0x620F)
$unicodeReports = -join @([char]0x62A5, [char]0x544A)
$tempRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase ('DTMAPI Player Doctor ' + $unicodeReadOnly + ' ' + $unicodeContains + ' space ' + [Guid]::NewGuid().ToString('N'))))
if (-not (Test-DtmApiPathIsSameOrChild -Child $tempRoot -Parent $tempBase)) { throw "Portable test root escaped system temp: $tempRoot" }
$lock = $null
try {
    $game = Join-Path $tempRoot ($unicodeGame + ' Game')
    $external = Join-Path $game 'BepInEx\plugins\External owner'
    $wrong = Join-Path $game 'BepInEx\plugins\Wrong CodeMod'
    $mods = Join-Path $game 'Mods'
    $reports = Join-Path $tempRoot ($unicodeReports + ' Reports')
