...
try {
    ...
    Assert-Rejected -Label 'pending switch without explicit mounted branch' -Action {
        Get-DtmApiSteamBranchIdentity -ManifestPath $conflictManifest -AllowPendingBranchSwitch
    }
    $pendingIdentity = Get-DtmApiSteamBranchIdentity `
        -ManifestPath $conflictManifest `
        -ExplicitBranch 'test' `
        -AllowPendingBranchSwitch
    if ($pendingIdentity.ResolvedBranch -cne 'test' -or
        $pendingIdentity.Source -cne 'appmanifest.MountedConfig(pending UserConfig mismatch)' -or
        $pendingIdentity.UserBranch -cne 'public' -or
        $pendingIdentity.MountedBranch -cne 'test' -or
        -not $pendingIdentity.PendingBranchSwitch) {
        throw 'Explicit pending branch switch did not preserve mounted/source/requested identity.'
    }
    Assert-Rejected -Label 'pending switch explicit branch must match MountedConfig' -Action {
        Get-DtmApiSteamBranchIdentity `
            -ManifestPath $conflictManifest `
            -ExplicitBranch 'public' `
            -AllowPendingBranchSwitch
    }
