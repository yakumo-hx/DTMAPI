...
try {
    ...
    Assert-True ($runnerSource.Contains('-RequireDisposableSaveRedirect $disposableSaveFixtureRequested') -and
        ...
        $runnerSource.Contains('LocalSave.dataDirPath redirect remains the sole save-storage owner')) `
    ...
    Assert-True (
        $runnerSource.Contains(
            "ValidateSet('None','U1','Prepare','U3Backpack','U3Mail','U4','MigratedSave')") -and
        $runnerSource.Contains(
        ...
        $nativeSaveRoutes.Contains(
            "MoreEquipmentSlotsTransitionPhase -ne 'U4'")) `
        'MoreEquipmentSlots U1/U3 must retain the real native sleep-save route while U4 remains NoNativeSave.'
    Assert-True `
        ($runnerSource.Contains(
    ...
    Assert-True (
        $qaHostSettingsSource.Contains(
            'U1/Prepare/U3Backpack/U3Mail/U4/MigratedSave phase')) `
        'MoreEquipmentSlots invalid transition-phase diagnostics must list MigratedSave.'
    Assert-True (
        $runnerSource.Contains(
    ...
    [System.IO.File]::WriteAllText(
        ...
        '{"schemaVersion":1,"disposable":true,"steamAutoCloudIsolated":true}',
        (New-Object System.Text.UTF8Encoding($false)))
    $archiveLink = Join-Path (Join-Path $archiveLinkFixture 'SAVE') 'ea-playtest-doloc-archive-2.data'
