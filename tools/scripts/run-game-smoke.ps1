param(
    ...
    [ValidateSet('None','U1','Prepare','U3Backpack','U3Mail','U4','MigratedSave')]
...
$qaAutomationRequested = [bool]$AutoSaveAfterLoad -or [bool]$AutoReloadMods -or
    ...
    [bool]$AutoExerciseMoreSavesOfficialSaveUi -or [bool]$AutoExerciseStrongPlantingGun -or
...
if ($AutoExerciseAutoFishingMovementCancel -and -not $AutoPressAutoFishingHotkey -and -not ($AutoFishingPerformance -and $AutoFishingPerformanceProfile -eq 'EnabledNoRod')) {
    throw '-AutoExerciseAutoFishingMovementCancel requires -AutoPressAutoFishingHotkey.'
}
...
if ($moreEquipmentSlotsTransitionRequested) {
    $transitionExpectedSaveMode =
        if ($MoreEquipmentSlotsTransitionPhase -eq 'U4') {
            'NoNativeSave'
        }
        else {
            'NativeSaveExpected'
    ...
    if (-not $StageQaHost -or
        ...
        throw '-MoreEquipmentSlotsTransitionPhase requires StageQaHost, explicit slot 3, DirectExe, SkipInstall, an isolated disposable fixture, title lifecycle, CoreOnly isolation, NativeSaveExpected for U1/Prepare/U3/MigratedSave and NoNativeSave for U4.'
    }
    $transitionExtraIds =
    ...
    elseif ($QaObserveEquipmentSlotsUi -or
        ...
        throw 'MoreEquipmentSlots Prepare/U3/U4 forbids the old consumer, Product, extra profile IDs, and the equipment UI observer.'
    }
...
if ($AssertMoreEquipmentSlotsColdRecovery) {
    if ((-not $PSBoundParameters.ContainsKey('SaveSlot')) -or $SaveSlot -ne 3) {
        throw '-AssertMoreEquipmentSlotsColdRecovery requires explicit -SaveSlot 3.'
    }
    if ($AutoExerciseMoreEquipmentSlots -or $AssertAdvancedProductOwnerDeactivation) {
        throw '-AssertMoreEquipmentSlotsColdRecovery is an independent product-disabled process and cannot combine with the enabled behavior or owner-deactivation route.'
    }
    if (-not $AutoExerciseTitleButtonLifecycle) {
        throw '-AssertMoreEquipmentSlotsColdRecovery requires -AutoExerciseTitleButtonLifecycle to prove title recovery before clean exit.'
    }
    if ($OfficialModProfile -ne 'CoreOnly' -or -not $IsolateAllOfficialMods) {
        throw '-AssertMoreEquipmentSlotsColdRecovery requires -OfficialModProfile CoreOnly and -IsolateAllOfficialMods so the product assembly is cold-disabled while Compatibility Host recovers durable state.'
    }
    if (-not $SkipInstall) {
        throw '-AssertMoreEquipmentSlotsColdRecovery requires -SkipInstall so the exact installed product can be disabled transactionally without an installer replacing the deployment tree.'
    }
}
...
$nativeSaveRouteRequested =
    ...
    ([bool]$moreEquipmentSlotsTransitionRequested -and
     $MoreEquipmentSlotsTransitionPhase -ne 'U4') -or
    [bool]$QaObserveSaveSaved
$archiveMutationRouteRequested = [bool]$AutoExerciseMoreSavesOfficialSaveUi
...
if (-not $qaRoutingProjectionOnly -and
    ($SaveTestMode -ne 'NoNativeSave' -or
     $disposableSaveFixtureRequested)) {
    if (-not $StageQaHost) {
        throw "$SaveTestMode requires -StageQaHost so the test-only LocalSave dataDirPath owner can prove exact disposable-fixture redirection."
    }
    if (-not $disposableSaveFixtureRequested) {
        throw "$SaveTestMode requires -DisposableSaveFixtureRoot."
    }
    if (-not $DirectExe -or $UseSteam) {
        throw "$SaveTestMode requires -DirectExe and forbids -UseSteam so the QA-only LocalSave.dataDirPath redirect remains the sole save-storage owner."
    }
...
function Test-SmokeNoQaReceiptComparison {
    ...
    return [pscustomobject]@{
        ...
        Passed = [bool]$unchanged.Passed -and -not [bool]$changed.Passed -and -not [bool]$emptyDirectoryChanged.Passed
    }
}
...
function Get-SmokeAuthorFileTreeDigest {
    param([Parameter(Mandatory = $true)] [string] $Path)

    $root = [System.IO.Path]::GetFullPath($Path).TrimEnd([char]92, [char]47)
    if (-not (Test-Path -LiteralPath $root -PathType Container)) {
        throw "Local11 author source tree does not exist: $root"
    }
    ...
    if (($rootItem.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -ne 0 -or
        ...
        throw "Local11 author source trees may not contain reparse points: $root"
    }
        ...
        foreach ($entry in $files.GetEnumerator()) {
            ...
            try {
                ...
                if ($fileStream.Position -ne $fileInfo.Length) {
                    throw "Local11 author source file changed while hashing: $($entry.Key)"
                }
            }
            finally {
                $fileStream.Dispose()
...
function Set-SmokeLocal11AuthorSourceState {
    param(
        [Parameter(Mandatory = $true)] [string] $GameDir,
        [Parameter(Mandatory = $true)] [string] $EvidencePath,
        [Parameter(Mandatory = $true)] [AllowEmptyCollection()] [string[]] $LocalIds,
        [switch] $AllowEmpty
    )

    $canonicalGameRoot = [System.IO.Path]::GetFullPath($GameDir).TrimEnd([char]92, [char]47)
    $persistentRoot = Get-DolocTownPersistentRootForSmoke
    $officialModsRoot = [System.IO.Path]::GetFullPath((Join-Path $persistentRoot 'MODS'))
    $requestedLocalIds = @($LocalIds | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | ForEach-Object { $_.Trim() } | Sort-Object -Unique)
    if ($requestedLocalIds.Count -eq 0 -and -not $AllowEmpty) {
        throw 'Local-product author source transaction requires at least one explicit Local.* Catalog id.'
    }
    $products = @(Get-SmokePublishedProductDefinitions | Where-Object {
        $requestedLocalIds -contains ('Local.' + [string]$_.officialFolder)
    } | Sort-Object uniqueId)
    if ($products.Count -ne $requestedLocalIds.Count) {
        $resolvedLocalIds = @($products | ForEach-Object { 'Local.' + [string]$_.officialFolder } | Sort-Object -Unique)
        $missingLocalIds = @($requestedLocalIds | Where-Object { $resolvedLocalIds -notcontains $_ })
        throw "Local-product author source transaction could not resolve every requested Catalog id: $($missingLocalIds -join '|')"
    }
    $selections = New-Object 'System.Collections.Generic.List[object]'
    foreach ($product in $products) {
        $codeModKindProperty = $product.PSObject.Properties['codeModKind']
        $isAdvancedProduct = $null -ne $codeModKindProperty -and
            [string]$codeModKindProperty.Value -ceq 'Advanced'
        $sourcePath = if ($isAdvancedProduct) {
            [System.IO.Path]::GetFullPath((Join-Path (Join-Path $canonicalGameRoot 'Mods') ([string]$product.uniqueId)))
        }
        else {
            [System.IO.Path]::GetFullPath((Join-Path $officialModsRoot ([string]$product.officialFolder)))
        }
        if ($isAdvancedProduct) {
            $receiptPath = Join-Path $sourcePath '.dtmapi-author-receipt.json'
            if (-not (Test-Path -LiteralPath $receiptPath -PathType Leaf)) {
                throw "Local-product author source transaction requires the Author SDK deployment receipt: $receiptPath"
            }
            $receipt = Get-Content -Raw -Encoding UTF8 -LiteralPath $receiptPath | ConvertFrom-Json
            if ([string]$receipt.uniqueId -cne [string]$product.uniqueId -or
                [string]$receipt.codeModKind -cne 'Advanced' -or
                [string]$receipt.destinationRelativePath -cne ('Mods/' + [string]$product.uniqueId)) {
                throw "Local-product author source transaction found a drifted Advanced deployment receipt for $($product.uniqueId)."
            }
        }
        $selections.Add([ordered]@{
            ...
            expectedTreeSha256 = Get-SmokeAuthorFileTreeDigest -Path $sourcePath
        }) | Out-Null
    }
    if ($selections.Count -ne $requestedLocalIds.Count) {
        throw "Local-product author source transaction selection count mismatch; expected=$($requestedLocalIds.Count); actual=$($selections.Count)."
    }

    $statePath = Get-SmokeAuthorSourceStatePath -GameDir $canonicalGameRoot
    $backupPath = Join-Path $EvidencePath 'local11-author-source-state.before.json'
    ...
    $summary = [ordered]@{
        ...
        RequestedLocalIds = @($requestedLocalIds)
    ...
    try {
        ...
        Write-SmokeJsonObject -Path (Join-Path $EvidencePath 'local11-author-source-state-summary.json') -Value $summary
        return $summary
    }
    catch {
        if ($originalExisted) {
...
function Restore-SmokeLocal11AuthorSourceState {
    param(
    ...
    Write-SmokeJsonObject -Path (Join-Path $EvidencePath 'local11-author-source-state-restore-verification.json') -Value ([ordered]@{
        StatePath = $statePath
...
function Get-SmokeWorkshopArtifactSnapshot {
    ...
    $expectedArtifact = $Product.retainedArtifact
    $snapshot = [ordered]@{
        ...
        ExpectedFileCount = [int]$expectedArtifact.fileCount
        ExpectedBytes = [int64]$expectedArtifact.bytes
        ExpectedTreeSha256 = [string]$expectedArtifact.treeSha256
        ActualFileCount = 0
        ActualBytes = [int64]0
        ActualTreeSha256 = ''
        ReparsePointPaths = @()
...
function Set-SmokeOfficialModProfile {
    ...
    # Mod discovery runs before the QA host can redirect LocalSave.dataDirPath.
    # A disposable save fixture therefore owns archive bytes only; the temporary
    # official-mod profile must still target the live pre-launch enablement file.
...
if ($ValidateNoQaUiEvidenceGateOnly) {
    ...
    [pscustomobject]@{
        ...
        Local11AuthorSourceStatePath = if ($OfficialModProfile -eq 'Local11') { Get-SmokeAuthorSourceStatePath -GameDir (Resolve-DolocTownGamePath -RepoRoot $repo) } else { '' }
        Local11AuthorSourceSelections = if ($OfficialModProfile -eq 'Local11') {
            $localModsRoot = Join-Path (Get-DolocTownPersistentRootForSmoke) 'MODS'
            @(Get-SmokePublishedProductDefinitions | Sort-Object uniqueId | ForEach-Object {
                $localRoot = [System.IO.Path]::GetFullPath((Join-Path $localModsRoot ([string]$_.officialFolder)))
                [ordered]@{
                    UniqueId = [string]$_.uniqueId
                    SourcePath = $localRoot
                    ExpectedTreeSha256 = Get-SmokeAuthorFileTreeDigest -Path $localRoot
                }
            })
        }
        else { @() }
        ...
        Passed = [bool]$comparisonSelfTest.Passed -and [bool]$animalInputSequenceSelfTest.Passed -and [bool]$deadlineSelfTest.Passed
...
$autoFishingToggleKeyEffective = if ($Batch6AutoFishingPilot -or ($Batch6AutoFishingManagerLifecycle -and $Batch6AutoFishingManagerMode -eq 'SameProcessDisable')) {
    'F6'
}
...
Write-SmokeJsonObject -Path (Join-Path $evidence 'save-test-mode.json') -Value ([ordered]@{
    ...
    QaIsolationOwner = if ($disposableSaveFixtureRequested) { 'qa-only LocalSave.dataDirPath exact Harmony owner' } else { 'none' }
...
if ($RequireExternalPlayerInputGate) {
    ...
    $externalPlayerInputRetainedArtifact = $externalPlayerInputProduct.retainedArtifact
    if ($null -eq $externalPlayerInputRetainedArtifact -or
        [string]$externalPlayerInputRetainedArtifact.entryDll -ne 'DTMAPI.YKeyConsole.dll' -or
        [string]$externalPlayerInputRetainedArtifact.entryDllSha256 -notmatch '^[A-Fa-f0-9]{64}$' -or
        [int]$externalPlayerInputRetainedArtifact.fileCount -le 0 -or
        [int64]$externalPlayerInputRetainedArtifact.bytes -le 0 -or
        [string]$externalPlayerInputRetainedArtifact.treeSha256 -notmatch '^[A-Fa-f0-9]{64}$') {
        throw 'External player-input Workshop artifact preflight requires the Catalog y-console retainedArtifact to freeze the old entry DLL, its SHA-256, and a non-empty exact file/byte/tree baseline.'
    }

    $expectedYConsoleWorkshopRoot = [System.IO.Path]::GetFullPath((Join-Path $externalWorkshopContentRoot '3742714442'))
    $expectedYConsoleWorkshopDll = Join-Path $expectedYConsoleWorkshopRoot ('Content\DTMAPI\' + [string]$externalPlayerInputRetainedArtifact.entryDll)
    $externalPlayerInputWorkshopArtifactSnapshot = Get-SmokeWorkshopArtifactSnapshot -Path $expectedYConsoleWorkshopRoot -Product $externalPlayerInputProduct

    $retainedRuntimeAuditPath = Join-Path $repo 'tools\release\baselines\retained-runtime-052-public-api-audit-20260715.json'
    if (-not (Test-Path -LiteralPath $retainedRuntimeAuditPath -PathType Leaf)) {
        throw "External player-input Workshop artifact preflight requires the tracked retained-runtime audit baseline: $retainedRuntimeAuditPath"
    }
    try {
        $retainedRuntimeAudit = Get-Content -Raw -Encoding UTF8 -LiteralPath $retainedRuntimeAuditPath | ConvertFrom-Json
    }
    catch {
        throw "External player-input Workshop artifact preflight could not parse the retained-runtime audit baseline: $($_.Exception.Message)"
    }
    $externalPlayerInputDllBaselines = @($retainedRuntimeAudit.retainedPublicProducts | Where-Object {
        [string]$_.productId -eq [string]$externalPlayerInputProduct.catalogId -and
        [string]$_.fileName -eq [string]$externalPlayerInputRetainedArtifact.entryDll
    })
    if (-not [bool]$retainedRuntimeAudit.overallPass -or $externalPlayerInputDllBaselines.Count -ne 1 -or
        [string]$externalPlayerInputDllBaselines[0].sha256 -notmatch '^[A-Fa-f0-9]{64}$') {
        throw "External player-input Workshop artifact preflight requires one passing retained-runtime DLL baseline for $($externalPlayerInputProduct.catalogId) / $($externalPlayerInputRetainedArtifact.entryDll)."
    }

    $expectedYConsoleWorkshopDllSha256 = ([string]$externalPlayerInputDllBaselines[0].sha256).ToUpperInvariant()
    $catalogYConsoleWorkshopDllSha256 = ([string]$externalPlayerInputRetainedArtifact.entryDllSha256).ToUpperInvariant()
    if (-not [string]::Equals(
        $expectedYConsoleWorkshopDllSha256,
        $catalogYConsoleWorkshopDllSha256,
        [System.StringComparison]::Ordinal)) {
        throw "External player-input Workshop artifact preflight found Catalog/audit SHA-256 drift for $($externalPlayerInputProduct.catalogId) / $($externalPlayerInputRetainedArtifact.entryDll)."
    }
    ...
    Write-SmokeJsonObject -Path (Join-Path $evidence 'external-input-artifact-preflight.json') -Value ([ordered]@{
        ...
        PackageDll = [string]$externalPlayerInputRetainedArtifact.entryDll
        RetainedRuntimeAuditPath = [System.IO.Path]::GetFullPath($retainedRuntimeAuditPath)
...
if ($publishedProductArtifactGateRequested) {
    ...
    Write-SmokeJsonObject -Path (Join-Path $evidence 'public-product-artifact-gate.json') -Value ([ordered]@{
        ...
        Passed = $publishedProductArtifactsOk
        Normalization = 'For each file: lowercase SHA-256, two spaces, forward-slash relative path; sort by relative path; join with LF; UTF-8 SHA-256.'
    })
...
if ($noNativeSaveMetadataRequested) {
    ...
    foreach ($currentArchiveIndex in $archiveIndexes) {
        foreach ($name in @(
            "ea-playtest-doloc-archive-$currentArchiveIndex.data",
            "ea-playtest-doloc-archive-$currentArchiveIndex-prev.data",
            "ea-playtest-doloc-archive-$currentArchiveIndex-bak.data"
        )) {
            $sourcePath = Join-Path $saveRoot $name
            $metadata = Get-SmokeFileMetadataSnapshot -Kind 'PlayerArchive' -Path $sourcePath
            $playerSaveSnapshots += [ordered]@{
                Name = $name
                ArchiveIndex = [int]$currentArchiveIndex
                Kind = [string]$metadata.Kind
                Path = [string]$metadata.Path
                Existed = [bool]$metadata.Existed
                Length = [int64]$metadata.Length
                Sha256 = [string]$metadata.Sha256
                LastWriteTimeUtc = [string]$metadata.LastWriteTimeUtc
                LastWriteTimeUtcTicks = [int64]$metadata.LastWriteTimeUtcTicks
            }
        }
    }
...
$qaHostStage = $null
$qaHostCleanup = $null
$moreEquipmentSlotsColdMarkerPath = ''
$moreEquipmentSlotsColdMarkerSha256 = ''
$moreEquipmentSlotsColdMarkerCreated = $false
$moreEquipmentSlotsColdMarkerRestored = -not [bool]$AssertMoreEquipmentSlotsColdRecovery
$moreEquipmentSlotsColdFixturePath = ''
$moreEquipmentSlotsColdFixtureStaged = $false
...
try {
if ($AssertMoreEquipmentSlotsColdRecovery) {
    $coldProductRoot = [System.IO.Path]::GetFullPath((Join-Path $gameDir 'Mods\DTMAPI.MoreEquipmentSlotsMod'))
    $coldReceiptPath = Join-Path $coldProductRoot '.dtmapi-author-receipt.json'
    if (-not (Test-Path -LiteralPath $coldProductRoot -PathType Container) -or
        -not (Test-Path -LiteralPath $coldReceiptPath -PathType Leaf)) {
        throw 'MoreEquipmentSlots cold recovery requires the exact Author SDK deployment and receipt in the game Mods directory.'
    }
    $coldReceipt = Get-Content -Raw -Encoding UTF8 -LiteralPath $coldReceiptPath | ConvertFrom-Json
    if ([string]$coldReceipt.uniqueId -cne 'DTMAPI.MoreEquipmentSlotsMod' -or
        [string]$coldReceipt.codeModKind -cne 'Advanced' -or
        [string]$coldReceipt.destinationRelativePath -cne 'Mods/DTMAPI.MoreEquipmentSlotsMod') {
        throw 'MoreEquipmentSlots cold recovery refused a drifted Author SDK deployment receipt.'
    }
    $moreEquipmentSlotsColdMarkerPath = Join-Path $coldProductRoot 'dtmapi.disabled'
    if (Test-Path -LiteralPath $moreEquipmentSlotsColdMarkerPath) {
        throw 'MoreEquipmentSlots cold recovery requires dtmapi.disabled to be absent before its reversible smoke transaction.'
    }
    try {
        $coldMarkerBytes = [System.Text.Encoding]::UTF8.GetBytes(
            'DTMAPI smoke-only MoreEquipmentSlots cold recovery marker; restore after process exit.')
        [System.IO.File]::WriteAllBytes($moreEquipmentSlotsColdMarkerPath, $coldMarkerBytes)
        $moreEquipmentSlotsColdMarkerSha256 = Get-SmokeFileSha256 -Path $moreEquipmentSlotsColdMarkerPath
        $moreEquipmentSlotsColdMarkerCreated = $true
        Write-SmokeJsonObject -Path (Join-Path $evidence 'more-equipment-slots-cold-marker-baseline.json') -Value ([ordered]@{
            ProductRoot = $coldProductRoot
            ReceiptPath = $coldReceiptPath
            MarkerPath = $moreEquipmentSlotsColdMarkerPath
            ExistedBefore = $false
            Created = $moreEquipmentSlotsColdMarkerCreated
            Length = [int64](Get-Item -LiteralPath $moreEquipmentSlotsColdMarkerPath).Length
            Sha256 = $moreEquipmentSlotsColdMarkerSha256
        })

        $moreEquipmentSlotsColdFixturePath = Join-Path $dtmapiDir 'config\protected-items\equipment-slots\slot-2\equipment-slots-DTMAPI.MoreEquipmentSlotsMod.json'
        if (Test-Path -LiteralPath $moreEquipmentSlotsColdFixturePath) {
            throw 'MoreEquipmentSlots cold recovery fixture path already exists; reversible staging was refused.'
        }
        New-Item -ItemType Directory -Force -Path (Split-Path -Parent $moreEquipmentSlotsColdFixturePath) | Out-Null
        Write-SmokeJsonObject -Path $moreEquipmentSlotsColdFixturePath -Value ([ordered]@{
            schemaVersion = 3
            scope = [ordered]@{
                archiveIndex = 2
                # The protected third-save fixture has no custom name, so the
                # native playerName getter resolves the current zh-CN default.
                playerName = [string]::Concat(
                    [char]0x666E,
                    [char]0x6D1B,
                    [char]0x7F07)
                customPlayerName = ''
            ...
            slots = @(
                [ordered]@{
                    index = 0
                    itemId = 'grandmas_button'
                    displayName = 'grandmas_button'
                    skillId = ''
                    defenseBonus = 0
                    isShield = $false
                    shieldValue = 0
        ...
        $moreEquipmentSlotsColdFixtureStaged = $true
        Write-SmokeJsonObject -Path (Join-Path $evidence 'more-equipment-slots-cold-fixture-stage.json') -Value ([ordered]@{
            FixturePath = $moreEquipmentSlotsColdFixturePath
            OwnerId = 'DTMAPI.MoreEquipmentSlotsMod'
            Schema = 'ProductNative-v3'
            ItemId = 'grandmas_button'
            SaveSlot = $SaveSlot
            ArchiveIndex = 2
            ConfigBaselineCapturedBeforeStage = $null -ne $g5ConfigDirectoryBaseline
            PlayerSaveBaselineCapturedBeforeStage = $playerSaveSnapshots.Count -gt 0
            Staged = $moreEquipmentSlotsColdFixtureStaged
        })
    }
    catch {
        if (Test-Path -LiteralPath $moreEquipmentSlotsColdMarkerPath -PathType Leaf) {
            Remove-Item -LiteralPath $moreEquipmentSlotsColdMarkerPath -Force
        }
        $moreEquipmentSlotsColdMarkerCreated = $false
        $moreEquipmentSlotsColdMarkerRestored = $true
        throw
    }
}
...
if ($OfficialModProfile -eq 'Local11') {
    $localAuthorSourceRequestedIds = @(Get-SmokePublishedProductLocalIds | Sort-Object -Unique)
}
else {
    $knownPublishedLocalIds = @(Get-SmokePublishedProductLocalIds | Sort-Object -Unique)
    $localAuthorSourceRequestedIds = @($requestedOfficialModProfileExtraIds | Where-Object { $knownPublishedLocalIds -contains $_ } | Sort-Object -Unique)
}
...
if ($localAuthorSourceRequested) {
    $local11AuthorSourceSummary = Set-SmokeLocal11AuthorSourceState -GameDir $gameDir -EvidencePath $evidence `
        -LocalIds $localAuthorSourceRequestedIds -AllowEmpty:$coreOnlyNoDemandSourceIsolation
    ...
    if (-not $local11AuthorSourceAppliedOk) {
        throw "Local-product author source transaction did not apply every validated LocalDevelopment selection; expected=$($localAuthorSourceRequestedIds.Count)."
    }
}
...
function Send-Batch6AutoFishingF6AfterHandshake {
    param(
        ...
        [ValidateSet('F6','A')] [string] $Key = 'F6',
        [string] $HookId = 'Smoke.Batch6.AutoFishingPilot.Handshake'
    )
    ...
    if ([bool]$handshake.Observed) {
        $sent = Send-DolocTownNamedKey -Key $Key -TimeoutSeconds $TimeoutSeconds -InputLogPath $LogPath
        $inputEvidence = $script:lastDolocTownInputEvidence
    }
    $provenancePassed = [bool]$handshake.Observed -and [bool]$sent -and
    ...
    $receipt = [ordered]@{
        ...
        HandshakeObserved = [bool]$handshake.Observed
        Key = $Key
        Sent = [bool]$sent
...
$moreEquipmentSlotsColdRecoveryDemandOk = -not [bool]$AssertMoreEquipmentSlotsColdRecovery
$moreEquipmentSlotsColdRecoveryHostOk = -not [bool]$AssertMoreEquipmentSlotsColdRecovery
$moreEquipmentSlotsColdRecoveryDestinationOk = -not [bool]$AssertMoreEquipmentSlotsColdRecovery
$moreEquipmentSlotsColdRecoveryOk = -not [bool]$AssertMoreEquipmentSlotsColdRecovery
...
if ($startupOk) {
    ...
    if ($gameLaunchedOk -and $AssertNoQaUiEvidence) {
        ...
        Write-Host ("NO-QA ORDINARY-PLAYER HANDSHAKE: no internal QA auto-load or ReturnHome navigation is active. Use normal player controls now: from the title screen select save slot 3. Waiting for SaveLoaded with {0}s remaining; shared runner deadline={1}." -f $noQaSaveLoadedWaitSeconds, $noQaRunnerDeadline.ToString('o')) -ForegroundColor Yellow
        "NoQaOrdinaryPlayerHandshake=$(Get-Date -Format o);shown=True;instruction=title-screen-to-save-slot-3;internalQaAutoLoad=False;internalQaReturnHome=False;remainingSeconds=$noQaSaveLoadedWaitSeconds;deadline=$($noQaRunnerDeadline.ToString('o'))" | Add-Content -LiteralPath (Join-Path $evidence 'summary.txt')
    }
    ...
    if ($saveLoadedOk -and $Batch6AutoFishingPilot) {
        ...
        if ($Batch6AutoFishingLevel -ne 'L0') {
            ...
            if ($batch6AutoFishingHandshakeOk -and $batch6AutoFishingInputOk -and $Batch6AutoFishingManualMovementCancel) {
                $movementCancel = Send-Batch6AutoFishingF6AfterHandshake -LogPath $logPath -Offset $batch6AutoFishingHandshakeOffset `
                    -State 'awaiting-manual-movement-a' -Key 'A' -TimeoutSeconds $TimeoutSeconds `
                    -HandshakeReceipts $batch6AutoFishingHandshakeReceipts -InputReceipts $batch6AutoFishingInputReceipts
            ...
            elseif ($batch6AutoFishingHandshakeOk -and $batch6AutoFishingInputOk) {
                $initialDisable = Send-Batch6AutoFishingF6AfterHandshake -LogPath $logPath -Offset $batch6AutoFishingHandshakeOffset `
                    -State 'awaiting-disable-f6' -TimeoutSeconds $TimeoutSeconds `
                    -HandshakeReceipts $batch6AutoFishingHandshakeReceipts -InputReceipts $batch6AutoFishingInputReceipts
            ...
            elseif ($batch6AutoFishingHandshakeOk -and $batch6AutoFishingInputOk -and $Batch6AutoFishingLevel -eq 'L5') {
                ...
                if ($batch6AutoFishingHandshakeOk -and $batch6AutoFishingInputOk) {
                    $finalDisable = Send-Batch6AutoFishingF6AfterHandshake -LogPath $logPath -Offset $batch6AutoFishingHandshakeOffset `
                        -State 'awaiting-final-disable-f6' -TimeoutSeconds $TimeoutSeconds `
                        -HandshakeReceipts $batch6AutoFishingHandshakeReceipts -InputReceipts $batch6AutoFishingInputReceipts
        ...
        $expectedBatch6HandshakeStates = if ($Batch6AutoFishingManualMovementCancel) {
            @('awaiting-initial-enable-f6','awaiting-manual-movement-a')
        }
        else { switch ($Batch6AutoFishingLevel) {
            'L0' { @() }
            'L4' { @('awaiting-initial-enable-f6','awaiting-disable-f6','l4-qa-recovery-active') }
            'L5' { @('awaiting-initial-enable-f6','awaiting-disable-f6','awaiting-title','awaiting-fifth-save-reentry','awaiting-reentry-enable-f6','awaiting-final-disable-f6') }
            default { @('awaiting-initial-enable-f6','awaiting-disable-f6') }
        } }
        $expectedBatch6InputStates = if ($Batch6AutoFishingManualMovementCancel) {
            ...
            'L0' { @() }
            'L5' { @('awaiting-initial-enable-f6','awaiting-disable-f6','awaiting-reentry-enable-f6','awaiting-final-disable-f6') }
            default { @('awaiting-initial-enable-f6','awaiting-disable-f6') }
        } }
        ...
        if ($Batch6AutoFishingManualMovementCancel) {
            ...
            $batch6AutoFishingInputOk = $batch6AutoFishingInputOk -and (($actualKeys -join '|') -ceq 'F6|A')
        }
        else {
            $batch6AutoFishingInputOk = $batch6AutoFishingInputOk -and
                @($batch6AutoFishingInputReceipts | Where-Object { [string]$_.Key -cne 'F6' }).Count -eq 0
        }
    ...
    if ($saveLoadedOk -and
        ...
        elseif ($MoreEquipmentSlotsTransitionPhase -ne 'U4') {
            ...
            if ($transitionSleepReady) {
                ...
                $transitionSleepSent =
                    Send-DolocTownKey `
                        ...
                        -Name ("MoreEquipmentSlots{0}Sleep{1}" -f $MoreEquipmentSlotsTransitionPhase, $transitionConfirmKey) `
                ...
                if (-not $transitionSleepSent -or
                    -not [bool]$transitionSleepEvidence.ForegroundMatchedAtSend -or
                    -not [bool]$transitionSleepEvidence.SendInputSucceeded -or
                    [bool]$transitionSleepEvidence.PostMessageFallbackUsed) {
                    $transitionSleepReady = $false
                }
            }
                ...
                if ($debugConsoleCloseYOk) {
                    ...
                    if ($debugConsoleTenYShortTapsOk) {
                        ...
                        if ($sentHoldY) {
                            ...
                            if (($RequireExternalPlayerInputGate -or $qaG4DebugConsoleEnabled -or $AssertNoQaUiEvidence) -and $debugConsoleHoldYNoFlickerOk -and $qaG4DebugEvidenceReadyOk) {
    ...
    if ($saveLoadedOk -and $AssertNoQaUiEvidence) {
        ...
        if ($noQaEquipmentCloseProvenanceOk) {
            $noQaEquipmentSlotsCloseOk = Wait-ForLogLineAfterOffsetUntilDeadline `
                -LogPath $logPath `
                -Offset ([int64]$noQaEquipmentCloseEvidence.LogOffsetAtSend) `
                -Pattern 'EquipmentSlots UI lifecycle cleared' `
                -Deadline (Get-SmokeCappedDeadline -Deadline $noQaUiDeadline -MaximumMilliseconds 20000) `
                -AbortOnFatalInstanceWindow
        }
        ...
        if ($AutoDriveNoQaAnimalViewer) {
            ...
            for ($animalOpenAttempt = 1; $animalOpenAttempt -le 2 -and $noQaAnimalViewerRenderReceiptCount -lt 1 -and (Test-SmokeDeadlineHasBudget -Deadline $noQaUiDeadline); $animalOpenAttempt++) {
                ...
                while ((Test-SmokeDeadlineHasBudget -Deadline $animalOpenAttemptDeadline) -and $noQaAnimalViewerRenderReceiptCount -lt 1) {
                    ...
                    $noQaAnimalViewerRenderReceiptCount = [regex]::Matches($animalTail, [regex]::Escape('Hook status: Animals.ViewerProgressUi = visible.')).Count
        ...
        if ($noQaAnimalViewerUiOk -and (Test-SmokeDeadlineHasBudget -Deadline $noQaUiDeadline)) {
            ...
            if ($noQaAnimalCloseProvenanceOk) {
                $noQaAnimalViewerNativeCloseOk = Wait-ForLogLineAfterOffsetUntilDeadline `
                    ...
                    -Pattern 'Animal viewer native lifecycle closed reason=native AnimalPanelUiState.Unregister.' `
                    -Deadline (Get-SmokeCappedDeadline -Deadline $noQaUiDeadline -MaximumMilliseconds 20000) `
                    -AbortOnFatalInstanceWindow
                $noQaAnimalViewerOverlayClearOk = Wait-ForLogLineAfterOffsetUntilDeadline `
                    -LogPath $logPath `
                    -Offset ([int64]$noQaAnimalCloseEvidence.LogOffsetAtSend) `
                    -Pattern 'Animal viewer progress overlay lifecycle session-cleared' `
                    -Deadline (Get-SmokeCappedDeadline -Deadline $noQaUiDeadline -MaximumMilliseconds 20000) `
                    -AbortOnFatalInstanceWindow
                if ($noQaAnimalViewerNativeCloseOk -and $noQaAnimalViewerOverlayClearOk) {
                    $animalCloseTail = Get-LogTextAfterOffset -LogPath $logPath -Offset ([int64]$noQaAnimalCloseEvidence.LogOffsetAtSend)
                    $nativeCloseIndex = $animalCloseTail.IndexOf('Animal viewer native lifecycle closed reason=native AnimalPanelUiState.Unregister.', [System.StringComparison]::Ordinal)
                    $overlayClearIndex = $animalCloseTail.IndexOf('Animal viewer progress overlay lifecycle session-cleared', [System.StringComparison]::Ordinal)
                    $noQaAnimalViewerCloseOrderOk = $nativeCloseIndex -ge 0 -and $overlayClearIndex -gt $nativeCloseIndex
                }
    ...
    if ($saveLoadedOk -and $AssertMoreEquipmentSlotsColdRecovery) {
...
if ($publishedProductArtifactGateRequested) {
    ...
    if (-not [bool]$recoveryExit.Exited) {
        ...
        if ($moreEquipmentSlotsColdMarkerCreated) {
            $manualRecoveryItems.Add([ordered]@{
                Kind = 'MoreEquipmentSlotsColdDisabledMarker'
                TargetPath = $moreEquipmentSlotsColdMarkerPath
                ExistedBefore = $false
                ActionAfterExit = 'RemoveOnlyIfLengthAndSha256Match'
                ExpectedLength = [int64](Get-Item -LiteralPath $moreEquipmentSlotsColdMarkerPath).Length
                ExpectedSha256 = $moreEquipmentSlotsColdMarkerSha256
            }) | Out-Null
        }
    ...
    if ($noNativeSaveMetadataRequested) {
        try {
            $playerSaveUnchangedBeforeCleanupOk = Compare-SmokeFileMetadataSnapshots `
                -Snapshots @($playerSaveSnapshots) `
                -EvidencePath (Join-Path $evidence 'player-save-unchanged-before-cleanup.json') `
                -SaveTestMode $SaveTestMode
    ...
    if ($null -ne $local11AuthorSourceSummary -and [bool]$local11AuthorSourceSummary.Applied -and -not $local11AuthorSourceRestored) {
        try {
            $local11AuthorSourceRestored = Restore-SmokeLocal11AuthorSourceState -Summary $local11AuthorSourceSummary -EvidencePath $evidence
    ...
    if ($moreEquipmentSlotsColdMarkerCreated) {
        try {
            if (-not (Test-Path -LiteralPath $moreEquipmentSlotsColdMarkerPath -PathType Leaf)) {
                throw 'MoreEquipmentSlots cold marker disappeared before restoration.'
            }
            $coldMarkerShaBeforeRemove = Get-SmokeFileSha256 -Path $moreEquipmentSlotsColdMarkerPath
            if (-not [string]::Equals($coldMarkerShaBeforeRemove, $moreEquipmentSlotsColdMarkerSha256, [System.StringComparison]::OrdinalIgnoreCase)) {
                throw 'MoreEquipmentSlots cold marker changed during the smoke run; automatic removal was refused.'
            }
            Remove-Item -LiteralPath $moreEquipmentSlotsColdMarkerPath -Force
            $moreEquipmentSlotsColdMarkerRestored = -not (Test-Path -LiteralPath $moreEquipmentSlotsColdMarkerPath)
            Write-SmokeJsonObject -Path (Join-Path $evidence 'more-equipment-slots-cold-marker-restore.json') -Value ([ordered]@{
                MarkerPath = $moreEquipmentSlotsColdMarkerPath
                ExpectedSha256 = $moreEquipmentSlotsColdMarkerSha256
                ActualSha256BeforeRemove = $coldMarkerShaBeforeRemove
                ExistsAfterRestore = Test-Path -LiteralPath $moreEquipmentSlotsColdMarkerPath
                Passed = $moreEquipmentSlotsColdMarkerRestored
            })
            if (-not $moreEquipmentSlotsColdMarkerRestored) {
                $restoreErrors.Add('more-equipment-slots-cold-marker:verification-failed') | Out-Null
            }
        }
        catch {
            $moreEquipmentSlotsColdMarkerRestored = $false
            $restoreErrors.Add("more-equipment-slots-cold-marker:$([string]$_.Exception.Message)") | Out-Null
        }
    }
    if ($null -ne $qaHostStage) {
        try {
...
if ($AssertNoQaUiEvidence) {
    ...
    $noQaExpectedInputLabels = @('Escape','Y1','Y2','Y3','YHold','YHoldCleanupEscape') + @(1..10 | ForEach-Object { "YShort$_" }) | Sort-Object
...
if ($AssertMoreEquipmentSlotsColdRecovery -and (Test-Path -LiteralPath $logPath -PathType Leaf)) {
    ...
    "MoreEquipmentSlotsColdRecovery=$moreEquipmentSlotsColdRecoveryOk;demand=$moreEquipmentSlotsColdRecoveryDemandOk;host=$moreEquipmentSlotsColdRecoveryHostOk;destination=$moreEquipmentSlotsColdRecoveryDestinationOk;productEntry=$([bool]$moreEquipmentSlotsColdProductEntry);failure=$([bool]$moreEquipmentSlotsColdFailure)" |
...
if ($AssertNoQaUiEvidence -and (Test-Path -LiteralPath $logPath -PathType Leaf)) {
    ...
    $noQaProductSource = if ($noQaUsesLocalProducts) { 'OfficialLocal' } else { 'Workshop' }
    ...
    foreach ($product in $noQaProducts) {
        ...
        $expectedSelectionPattern = 'Duplicate UniqueID ' + $escapedOwnerId + ' discovered; using ' + $noQaProductSource + ' enabled=true root=' + [regex]::Escape($expectedProductRoot) + '(?:[.;|]|$)'
        ...
        if ($noQaUsesLocalProducts) {
            ...
            $expectedLoadSourcePattern = 'Code mod load-source owner=' + $escapedOwnerId + '; source=OfficialLocal; workshopId=none; root=' +
...
$result = @{
    ...
    MoreEquipmentSlotsRetainedWorkshopArtifact =
        $moreEquipmentSlotsRetainedWorkshopArtifactSnapshot
    MoreEquipmentSlotsColdRecovery = Get-SmokeStatus -Requested ([bool]$AssertMoreEquipmentSlotsColdRecovery) -Passed $moreEquipmentSlotsColdRecoveryOk
    MoreEquipmentSlotsColdRecoveryDemand = Get-SmokeStatus -Requested ([bool]$AssertMoreEquipmentSlotsColdRecovery) -Passed $moreEquipmentSlotsColdRecoveryDemandOk
    MoreEquipmentSlotsColdRecoveryHost = Get-SmokeStatus -Requested ([bool]$AssertMoreEquipmentSlotsColdRecovery) -Passed $moreEquipmentSlotsColdRecoveryHostOk
    MoreEquipmentSlotsColdRecoveryDestination = Get-SmokeStatus -Requested ([bool]$AssertMoreEquipmentSlotsColdRecovery) -Passed $moreEquipmentSlotsColdRecoveryDestinationOk
