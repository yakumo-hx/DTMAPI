...
try {
    ...
    Assert-NoDemandTest (-not $wrapperSource.Contains("Join-Path `$gameRoot 'Mods'") -and
        -not $wrapperSource.Contains("'.dtmapi-author-receipt.json'") -and
        -not $wrapperSource.Contains("'dtmapi.disabled'") -and
        -not $wrapperSource.Contains('Start-NoDemandManagedProductIsolation') -and
        -not $wrapperSource.Contains('Complete-NoDemandManagedProductIsolation') -and
        -not $wrapperSource.Contains('RecoverManagedProductIsolationOnly') -and
        $wrapperSource.Contains('Batch 5 no-demand acceptance requires DolocTown.exe to be absent before launch.')) 'The no-demand wrapper must isolate functional products only through the official CoreOnly profile and must not inspect or mutate retired <game>/Mods Author deployments.'
