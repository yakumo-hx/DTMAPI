...
if ($null -ne $contract) {
    ...
    $defaultRuntimeReceiptFullPath = Resolve-RepoFile ([string]$runtimeAudit.receiptPath) ([string]$runtimeAudit.receiptPath)
    $requestedRuntimeReceiptFullPath = Resolve-RepoFile $runtimeReceiptRelativePath ([string]$runtimeAudit.receiptPath)
    if ($isPassed) {
        if (-not $requestedRuntimeReceiptFullPath.Equals($defaultRuntimeReceiptFullPath, [StringComparison]::OrdinalIgnoreCase)) {
            Add-Failure "Passed G2 must use the contract-owned Runtime receipt path '$($runtimeAudit.receiptPath)'; external or alternate receipt overrides are development-only."
        }
        $runtimeReceiptRelativePath = [string]$runtimeAudit.receiptPath
        $runtimeReceiptFullPath = $defaultRuntimeReceiptFullPath
        Assert-TrackedAndClean ([string]$runtimeAudit.receiptPath)
    }
    else {
        $runtimeReceiptFullPath = $requestedRuntimeReceiptFullPath
    }
    ...
    if ($null -ne $candidate) {
        if ($isPassed) {
            ...
            $defaultReceiptFullPath = Resolve-RepoFile ([string]$audit.receiptPath) ([string]$audit.receiptPath)
            $requestedReceiptFullPath = Resolve-RepoFile $receiptRelativePath ([string]$audit.receiptPath)
            if (-not $requestedReceiptFullPath.Equals($defaultReceiptFullPath, [StringComparison]::OrdinalIgnoreCase)) {
                Add-Failure "Passed G2 must use the contract-owned ownership receipt path '$($audit.receiptPath)'; external or alternate receipt overrides are development-only."
            }
            $receiptRelativePath = [string]$audit.receiptPath
            $receiptFullPath = $defaultReceiptFullPath
            Assert-TrackedAndClean ([string]$audit.receiptPath)
            ...
            if (Test-GitCommand @('ls-files', '--error-unmatch', '--', (Normalize-Path $receiptRelativePath))) {
                ...
                $historicalClosureGovernancePaths = @()
                try {
                    $historicalContract = Read-GitText $receiptCommit $contractRelativePath | ConvertFrom-Json
                }
                catch {
                    Add-Failure "G2 historical closure contract is not parseable at $receiptCommit`: $($_.Exception.Message)"
                }
                if ($null -ne $historicalContract) {
                    Assert-Exact 'Historical closure contract schema version' $historicalContract.schemaVersion '2'
                    Assert-Exact 'Historical closure contract ID' $historicalContract.contractId ([string]$contract.contractId)
                    Assert-Exact 'Historical closure Phase 0 admission commit' $historicalContract.phaseState.phase0AdmissionCommit $admissionCommit
                    Assert-Exact 'Historical closure G2 state' $historicalContract.phaseState.g2Implementation 'passed'
                    Assert-Exact 'Historical closure G2 implementation commit' $historicalContract.phaseState.g2ImplementationCommit $targetCommit
                    Assert-Exact 'Historical closure synthetic fixture state' $historicalContract.phaseState.g2SyntheticFixture 'passed'
                    Assert-Exact 'Historical closure architecture authority' $historicalContract.authorities.architecture ([string]$receipt.architectureAuthority)
                    Assert-Exact 'Historical closure review authority' $historicalContract.authorities.review ([string]$receipt.reviewAuthority)
                    Assert-Exact 'Historical closure ownership receipt path' $historicalContract.ownershipAudit.receiptPath ([string]$audit.receiptPath)
                    Assert-Exact 'Historical closure ownership baseline commit' $historicalContract.ownershipAudit.baselineCommit ([string]$receipt.baselineCommit)
                    Assert-Exact 'Historical closure ownership receipt schema' $historicalContract.ownershipAudit.receiptSchemaVersion ([string]$receipt.schemaVersion)
                    Assert-Exact 'Historical closure Runtime receipt path' $historicalContract.runtimeAudit.receiptPath ([string]$runtimeAudit.receiptPath)
                    Assert-Exact 'Historical closure Runtime implementation commit' $historicalContract.runtimeAudit.implementationCommit ([string]$runtimeReceipt.implementationCommit)
                    Assert-Exact 'Historical closure Runtime matrix SHA' $historicalContract.runtimeAudit.matrixSha256 ([string]$runtimeReceipt.matrixSha256)
                    Assert-ExactSet 'Historical closure Runtime case set' @($historicalContract.runtimeAudit.requiredCaseIds) @($runtimeReceipt.cases | ForEach-Object { [string]$_.caseId })
                    if ($null -ne $historicalContract.PSObject.Properties['runtime055Amendment']) {
                        Add-Failure 'The immutable G2 closure contract unexpectedly contains the later Runtime 0.5.5 amendment.'
                    }
                    $historicalClosureGovernancePaths = @($historicalContract.runtimeAudit.closureGovernancePaths | ForEach-Object { Normalize-Path ([string]$_) })
                    Assert-ExactSet 'Historical/current G2 closure governance path set' $historicalClosureGovernancePaths @($runtimeAudit.closureGovernancePaths | ForEach-Object { Normalize-Path ([string]$_) })
                    $historicalContractBlob = ([string](Invoke-GitLines @('rev-parse', ($receiptCommit + ':' + $contractRelativePath)) | Select-Object -First 1)).Trim()
                    if ([string]::IsNullOrWhiteSpace($historicalContractBlob) -or
                        -not (Test-GitCommand @('cat-file', '-e', ($historicalContractBlob + '^{blob}')))) {
                        Add-Failure "G2 historical closure contract blob is not resolvable: $historicalContractBlob"
                    }
                    else {
                        Write-Host "G2 historical closure contract verified: closure=$receiptCommit; blob=$historicalContractBlob."
                    }
                }
                foreach ($governancePath in $historicalClosureGovernancePaths) {
                ...
                Assert-ExactSet 'G2 implementation-to-closure changed paths' $closureImplementationDrift $historicalClosureGovernancePaths
            }
            else {
                Add-Failure "Passed G2 ownership receipt must be tracked so its historical closure commit and blob can be verified: $receiptRelativePath"
            }
        }
        else {
