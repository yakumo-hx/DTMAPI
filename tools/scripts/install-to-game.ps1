...
trap {
    ...
    if (-not $script:DtmInstallFailureStateAllowed) {
        Write-Warning "DTMAPI install was blocked by deterministic product preflight before any Runtime, installer-state, official MODS, or enablement mutation: $($installErrorRecord.Exception.Message)"
        if ($null -ne $script:DtmInstallerMutationLock) {
...
if (-not $DryRun) {
    $script:DtmInstallLockAcquisitionPending = $true
    Assert-DtmApiGameNotRunning -GameDir $gameDir -Operation 'install'
    Assert-DtmApiGameDirectoryMutationRoot -GameDir $gameDir
    $script:DtmInstallerMutationLock = Enter-DtmApiInstallerMutationLock -GameDir $gameDir -Operation 'install'
    $script:DtmInstallLockAcquisitionPending = $false
}
...
$unexpectedPlayerDoctorSourceRoot = Join-Path $installerToolSourceRoot 'player-doctor'
if ($usingPackagePayload -and (Test-Path -LiteralPath $unexpectedPlayerDoctorSourceRoot)) {
    throw "The normal Runtime Workshop package must not contain the optional Player Doctor payload: $unexpectedPlayerDoctorSourceRoot"
}
...
function Get-DtmApiRuntimeAssemblyMetadata {
    ...
    foreach ($fileName in $runtimeFiles) {
        ...
        $sha256 = (Get-DtmApiFileSha256 -Path $path).ToLowerInvariant()
        if (-not [string]::IsNullOrWhiteSpace($ReferenceRoot)) {
            ...
            $referenceHash = (Get-DtmApiFileSha256 -Path $referencePath).ToLowerInvariant()
...
function Initialize-DtmApiRuntimeInstallCandidate {
    ...
    if (-not [string]::IsNullOrWhiteSpace($assetSource)) {
        ...
        $Transaction.AssetRecord = [ordered]@{
            ...
            Sha256 = (Get-DtmApiFileSha256 -Path $candidateAsset).ToLowerInvariant()
        }
    }
    else {
    ...
    Write-Host '[INFO] Player Doctor is not part of the Runtime install transaction.'
...
function Assert-DtmApiCommittedRuntimeInstall {
    ...
    foreach ($expected in @($Transaction.OptionalComponentMetadata)) {
        ...
        $componentHash = Get-DtmApiFileSha256 -Path $componentPath
    ...
    foreach ($record in @($Transaction.StateToolRecords)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($expectedHash)) {
            $actualHash = Get-DtmApiFileSha256 -Path $recordPath
        ...
        if ($recordKind -eq 'player-doctor' -or $recordKind -eq 'player-doctor-license') {
            throw "The Runtime install committed an unexpected Player Doctor receipt: $recordPath"
        }
    }
    $committedPlayerDoctorRoot = Join-Path $Transaction.LiveTools 'player-doctor'
    if (Test-Path -LiteralPath $committedPlayerDoctorRoot) {
        throw 'The Runtime install committed an unexpected Player Doctor directory.'
    }
...
function Assert-DtmApiNoPausedAuthorSdkInstall {
    param(
        [Parameter(Mandatory = $true)] [AllowEmptyCollection()] [object[]] $Definitions
    )

    $pausedIds = @($Definitions | Where-Object {
        Test-DtmApiDefinitionFlag -Mod $_ -Key 'AuthorSdkProject'
...
Assert-DtmApiNoPausedAuthorSdkInstall -Definitions $officialLocalMods

if ($DryRun) {
    $legacyDetections = @(Get-DtmApiLegacyDetections -GameDir $gameDir)
    $plannedOfficialMods = @($officialLocalMods)
...
if ($usingPackagePayload) {
    ...
    if ($KeepLegacyMigratedGameMods) {
        Write-Host 'Keeping the complete existing game Mods tree for legacy deployment recovery/withdraw compatibility.'
    }
    else {
...
function Get-DtmApiPackageTreeFingerprint {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $rootFull -File -Recurse -Force | Sort-Object FullName)) {
        ...
        $hash = (Get-DtmApiFileSha256 -Path $file.FullName).ToLowerInvariant()
