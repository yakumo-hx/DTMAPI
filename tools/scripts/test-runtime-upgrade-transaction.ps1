...
function New-RuntimeTransactionPayload {
    ...
    $playerDoctorSource = Join-Path $repo 'dist\player-doctor\win-x64'
    $playerDoctorDestination = Join-Path $toolsRoot 'player-doctor'
    New-Item -ItemType Directory -Force -Path $playerDoctorDestination | Out-Null
    foreach ($playerDoctorFile in @('dtmapi-player-doctor.exe', 'dotnet-LICENSE.txt', 'dotnet-ThirdPartyNotices.txt')) {
        Copy-Item -LiteralPath (Join-Path $playerDoctorSource $playerDoctorFile) -Destination (Join-Path $playerDoctorDestination $playerDoctorFile) -Force
    }
...
function Assert-RuntimeTransactionSuccess {
    ...
    $payloadPlayerDoctorRoot = Join-Path ([System.IO.Path]::GetFullPath((Join-Path $Fixture.PayloadRoot '..\tools'))) 'player-doctor'
    $actualPlayerDoctorFiles = @(Get-ChildItem -LiteralPath $livePlayerDoctorRoot -File -Recurse | ForEach-Object {
        $_.FullName.Substring(([System.IO.Path]::GetFullPath($livePlayerDoctorRoot)).TrimEnd('\').Length).TrimStart('\').Replace('\', '/')
    } | Sort-Object)
    Assert-RuntimeTransactionTest -Condition (($actualPlayerDoctorFiles -join '|') -eq ($expectedPlayerDoctorFiles -join '|')) -Message "$Label did not install the exact Player Doctor file set."
    foreach ($playerDoctorFile in $expectedPlayerDoctorFiles) {
        $livePath = Join-Path $livePlayerDoctorRoot $playerDoctorFile
        $payloadPath = Join-Path $payloadPlayerDoctorRoot $playerDoctorFile
        $liveHash = (Get-FileHash -LiteralPath $livePath -Algorithm SHA256).Hash
        $payloadHash = (Get-FileHash -LiteralPath $payloadPath -Algorithm SHA256).Hash
        Assert-RuntimeTransactionTest -Condition ($liveHash -eq $payloadHash) -Message "$Label installed the wrong Player Doctor bytes for $playerDoctorFile."
    }
    $playerDoctorVersion = [System.Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $livePlayerDoctorRoot 'dtmapi-player-doctor.exe'))
    Assert-RuntimeTransactionTest -Condition (
        [string]::Equals([string]$playerDoctorVersion.ProductVersion, $script:DtmApiReleaseVersion, [System.StringComparison]::Ordinal) -and
        [string]::Equals([string]$playerDoctorVersion.FileVersion, $script:DtmApiBinaryVersion, [System.StringComparison]::Ordinal)
    ) -Message "$Label installed Player Doctor version $($playerDoctorVersion.ProductVersion)/$($playerDoctorVersion.FileVersion), expected $script:DtmApiReleaseVersion/$script:DtmApiBinaryVersion."
    ...
    Assert-RuntimeTransactionTest -Condition ($playerDoctorStateRecords.Count -eq $expectedPlayerDoctorFiles.Count) -Message "$Label install state did not retain all Player Doctor hash receipts."
    foreach ($record in $playerDoctorStateRecords) {
        $recordPath = [System.IO.Path]::GetFullPath([string]$record.Path)
        $recordHash = [string]$record.Sha256
        Assert-RuntimeTransactionTest -Condition ($recordHash.Length -eq 64) -Message "$Label Player Doctor receipt omitted SHA-256 for $recordPath."
        Assert-RuntimeTransactionTest -Condition ([string]::Equals((Get-FileHash -LiteralPath $recordPath -Algorithm SHA256).Hash, $recordHash, [System.StringComparison]::OrdinalIgnoreCase)) -Message "$Label Player Doctor receipt hash is wrong for $recordPath."
    }
    ...
    Assert-RuntimeTransactionTest -Condition ($cleanStatusText -match 'Installed Player Doctor file set is exact\.') -Message "$Label status check did not recognize the exact installed Player Doctor footprint. Output=$($cleanStatus.Output -join ' | ')"
    Assert-RuntimeTransactionTest -Condition ($cleanStatusText -match 'Checked recorded Player Doctor hashes: 3') -Message "$Label status check did not verify all three Player Doctor hash receipts. Output=$($cleanStatus.Output -join ' | ')"
    ...
    $tamperedLicense = Join-Path $livePlayerDoctorRoot 'dotnet-LICENSE.txt'
    Write-RuntimeTransactionText -Path $tamperedLicense -Text 'tampered-player-doctor-license'
    $tamperedStatus = Invoke-RuntimeTransactionStatus -Fixture $Fixture
    $tamperedStatusText = $tamperedStatus.Output -join "`n"
    Assert-RuntimeTransactionTest -Condition ($tamperedStatusText -match 'Install-state Player Doctor receipt hash mismatch: dotnet-LICENSE\.txt') -Message "$Label status check did not reject a tampered installed Player Doctor file. Output=$($tamperedStatus.Output -join ' | ')"
    Copy-Item -LiteralPath (Join-Path $payloadPlayerDoctorRoot 'dotnet-LICENSE.txt') -Destination $tamperedLicense -Force

    $missingNotice = Join-Path $livePlayerDoctorRoot 'dotnet-ThirdPartyNotices.txt'
    Remove-Item -LiteralPath $missingNotice -Force
    $missingStatus = Invoke-RuntimeTransactionStatus -Fixture $Fixture
    $missingStatusText = $missingStatus.Output -join "`n"
    Assert-RuntimeTransactionTest -Condition ($missingStatusText -match 'Installed Player Doctor file set is missing, extra, or nested incorrectly\.') -Message "$Label status check did not reject a missing installed Player Doctor file. Output=$($missingStatus.Output -join ' | ')"
    Copy-Item -LiteralPath (Join-Path $payloadPlayerDoctorRoot 'dotnet-ThirdPartyNotices.txt') -Destination $missingNotice -Force
    Assert-RuntimeTransactionTest -Condition ([System.IO.File]::ReadAllText((Join-Path $Fixture.StateDir 'reports\keep-report.txt')) -eq 'unrelated-report-sentinel') -Message "$Label changed unrelated report state."
    Assert-RuntimeTransactionTest -Condition ([System.IO.File]::ReadAllText((Join-Path $Fixture.StateDir 'config\keep-config.json')) -eq '{"keep":true}') -Message "$Label changed unrelated config state."
    Assert-NoRuntimeTransactionResidue -Fixture $Fixture -Label $Label
