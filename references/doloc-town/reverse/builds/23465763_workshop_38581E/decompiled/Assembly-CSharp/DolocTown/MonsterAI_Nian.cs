...
[MonsterAI("nian")]
public class MonsterAI_Nian : MonsterAI
{
	public class WanderState : MonsterStateManager.MoveState<MonsterAI_Nian>
	{
	...
	public class AttackState : MonsterStateManager.AttackState<MonsterAI_Nian>
	{
		public override MonsterStateManager.MonsterState NextState(float dt)
		{
			if (base.IsAttacking)
	...
	public class IdleState : MonsterStateManager.MoveState<MonsterAI_Nian>
	{
		...
		public override void OnEnter()
		{
			...
			Path = new Vector2Int[num + 1];
			Vector2Int vector2Int = _controller.Env.AirMap.WorldToCell(_controller.PositionCenter);
			Path[0] = vector2Int;
			for (int i = 1; i < Path.Length; i++)
			{
				Vector2Int vector2Int2 = Path[i - 1];
				if (i % 3 == 0)
				{
					vector2Int2.x++;
				}
				vector2Int2.y += ((i % 2 == 1) ? 3 : (-3));
				Path[i] = vector2Int2;
			}
			((MonsterMoverContextSteering)_controller.mover).SetOverridenPath(Path);
			isMoving = _controller.MoveTo(MoveTargetType.Around);
		}
	}

	public class PursuitState : MonsterStateManager.MoveState<MonsterAI_Nian>
	{
		public override MonsterStateManager.MonsterState NextState(float dt)
		{
			if (isMoving)
	...
	private bool ShouldPursuitTarget
	{
		get
		{
			MonsterAttackId attackId;
			if (LockedTarget != null)
			{
				return _controller.IsAnyAttackBehaviourAvailable(out attackId);
			}
			return false;
		}
	}
