...
public class DungeonResourceModelMechanicalGarbage : DungeonResource
{
	public DungeonResourceModelMechanicalGarbage(IDungeonResourceHost host, ResourceInfo proto, Vector3 wp, Vector2Int anchor)
		: base(host, proto, wp, anchor)
	{
	}

	[JsonConstructor]
	protected DungeonResourceModelMechanicalGarbage(int id, Vector2Int anchor, Vector3 position, string ResourceName, int GrowthLevel, int currentGrowth, int currentHealth, int skinIdx, int randomSeed)
	...
	{
		DolocAPI.effectProvider.RaiseInstPS(hitPosition, InstantParticleEffectsType.SMOKE_BRUST_02);
	}
}
