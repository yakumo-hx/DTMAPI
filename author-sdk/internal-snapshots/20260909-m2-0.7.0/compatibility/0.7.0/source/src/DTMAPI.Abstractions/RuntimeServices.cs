...
namespace DTMAPI.Abstractions
{
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Acquire on the Runtime thread. Snapshot reads and IsMainThread are safe on any thread. Change callbacks run on the Runtime thread after publication; Dispose only unsubscribes.")]
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Acquire on the Runtime thread, then submit from any thread. Synchronous Action only; callbacks are never inline. Completion continuations are asynchronous and not guaranteed on the Runtime thread. Never block that thread on Completion. Cancellation/queueTimeout affect unstarted work only.")]
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Unregister releases platform ownership without disposing. Dispose requests Runtime-thread disposal; failed disposal may be retried. Neither operation reactivates a closed registration.")]
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Registration is Runtime-thread only. All frontends execute through the shared scheduler. Canonical names are UniqueID/name; alias conflicts reject the entire registration. Input is data, never shell code.")]
