...
namespace DTMAPI.Abstractions
{
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Read-only files from this Mod's selected package; runtime-thread and active-owner only. Relative keys, strict UTF-8 and bounded file size.")]
    public interface IOwnerFileHelper
    {
        OwnerDataResult<string> ReadText(string key);
    }

    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.7.0", Notes = "Local global data, independent of gameplay saves and package lifetime. Found indicates a successful read/write/delete; no native save or Cloud guarantee.")]
    ...
    {
        public static IOwnerFileHelper GetOwnerFiles(this IDtmHelper helper) => helper.GetRequiredService<IOwnerFileHelper>("0.7.0", "0.7.0");
        public static IGlobalDataHelper GetGlobalData(this IDtmHelper helper) => helper.GetRequiredService<IGlobalDataHelper>("0.7.0", "0.7.0");
    }
}
