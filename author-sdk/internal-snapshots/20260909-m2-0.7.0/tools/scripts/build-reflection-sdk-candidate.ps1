...
$repo = Get-RepoRoot
if (@((Get-AuthorSdkTargetCatalog -RepoRoot $repo).targets | Where-Object { $_.apiTarget -ceq '0.8.0' -and $_.state -ceq 'available' }).Count) { throw 'PN-021 staging is only valid before its target freeze. Use build-author-sdk.ps1 for the available target.' }
...
if ((Get-AuthorSdkSha256 -Path (Join-Path $ordinary 'compatibility/0.7.0/DTMAPI.Abstractions.dll')) -ne '69a28c5c8da3a9b19afdff9d7a12c728b44161c31ffc269e790b1b270b16cfd4') { throw 'Ordinary frozen payload mismatch.' }
[IO.Directory]::CreateDirectory($output) | Out-Null
$source = Join-Path $output 'source'
$sdk = Join-Path $output 'DTMAPI-Author-SDK-0.3.0-candidate-win-x64'
...
Edit-CandidateText 'tools/release/dtmapi-runtime-version.props' '<DtmApiReleaseVersion>0.7.0</DtmApiReleaseVersion>' '<DtmApiReleaseVersion>0.8.0</DtmApiReleaseVersion>'
Edit-CandidateText 'tools/release/dtmapi-runtime-version.props' '<DtmApiBinaryFileVersion>0.7.0.0</DtmApiBinaryFileVersion>' '<DtmApiBinaryFileVersion>0.8.0.0</DtmApiBinaryFileVersion>'
Edit-CandidateText 'src/DTMAPI.Authoring.Contracts/AuthorContracts.cs' 'const string SdkVersion = "0.2.0"' 'const string SdkVersion = "0.3.0"'
Edit-CandidateText 'src/DTMAPI.Authoring.Contracts/AuthorContracts.cs' 'const string HighestSupportedRuntimeVersion = "0.7.0"' 'const string HighestSupportedRuntimeVersion = "0.8.0"'
foreach ($project in @('DTMAPI.AuthorSdk','DTMAPI.Authoring.Contracts','DTMAPI.InstallDoctor','DTMAPI.Tooling.Metadata')) {
    Edit-CandidateText "src/$project/$project.csproj" '0.2.0' '0.3.0'
}
Edit-CandidateText 'src/DTMAPI.AuthorSdk/AuthorApplication.cs' 'DTMAPI Author SDK 0.2.0 - frozen API targets' 'DTMAPI Author SDK 0.3.0 candidate - isolated API targets'
...
$payload = Join-Path $output 'compatibility-0.8.0'
Copy-Item -LiteralPath (Join-Path $ordinary 'compatibility/0.7.0') -Destination $payload -Recurse
Copy-Item -LiteralPath $abstractions -Destination (Join-Path $payload 'DTMAPI.Abstractions.dll') -Force
$contract = Get-Content (Join-Path $source 'author-sdk/compatibility/0.7.0/compatibility.contract.json') -Raw | ConvertFrom-Json
$contract.sdkVersion = '0.3.0'
$contract.targetRuntimeVersion = '0.8.0'
...
$contractDir = Join-Path $source 'author-sdk/compatibility/0.8.0'
...
$base = $catalog.targets | Where-Object apiTarget -eq '0.7.0'
$next = ($base | ConvertTo-Json -Depth 8 | ConvertFrom-Json)
$next.apiTarget = '0.8.0'
$next.minimumRuntimeVersion = '0.8.0'
$next.maximumPackageRuntimeVersion = '0.8.0'
$next.payloadSdkVersion = '0.3.0'
$next.sdkVersions = @('0.3.0')
$next.payloadPath = 'compatibility/0.8.0'
$next.contractPath = 'compatibility/0.8.0/compatibility.contract.json'
$next.contractResourceName = 'DTMAPI.AuthorSdk.compatibility.0.8.0.contract.json'
$catalog.targets = @($catalog.targets) + @($next)
$catalog.defaultTarget = '0.8.0'
($catalog.targets | Where-Object apiTarget -eq '0.5.5').sdkVersions = @('0.1.0','0.2.0','0.3.0')
$old = $catalog.targets | Where-Object apiTarget -eq '0.7.0'
$old.sdkVersions = @('0.2.0','0.3.0')
$next = $catalog.targets | Where-Object apiTarget -eq '0.8.0'
$next.state = 'available'
$next.contractSha256 = Get-AuthorSdkSha256 -Path $contractPath
$next.capabilities = @('optional-helper-services/1','runtime-context/1','scheduler/1','owned-resources/1','commands/1','owner-files/1','global-data/1','versioned-config/1','input-diagnostics/1','translations/1','reflection/1')
...
Edit-CandidateText 'src/DTMAPI.AuthorSdk/DTMAPI.AuthorSdk.csproj' '</Project>' @'
  <ItemGroup>
    <EmbeddedResource Include="../../author-sdk/compatibility/0.8.0/compatibility.contract.json" LogicalName="DTMAPI.AuthorSdk.compatibility.0.8.0.contract.json" />
  </ItemGroup>
</Project>
'@
...
$legacyDestination = Join-Path $sdk 'compatibility/0.7.0'
[IO.Directory]::CreateDirectory($legacyDestination) | Out-Null
foreach ($item in Get-ChildItem -LiteralPath (Join-Path $ordinary 'compatibility/0.7.0')) {
    Copy-Item -LiteralPath $item.FullName -Destination (Join-Path $legacyDestination $item.Name) -Recurse -Force
}
Copy-Item -LiteralPath $payload -Destination (Join-Path $sdk 'compatibility/0.8.0') -Recurse
[IO.Directory]::CreateDirectory((Join-Path $sdk 'contracts')) | Out-Null
Copy-Item -LiteralPath $contractPath -Destination (Join-Path $sdk 'contracts/compatibility-0.8.0.contract.json')
Copy-Item -LiteralPath (Join-Path $source 'author-sdk/compatibility/0.7.0/compatibility.contract.json') -Destination (Join-Path $sdk 'contracts/compatibility-0.7.0.contract.json')
...
[IO.Directory]::CreateDirectory((Join-Path $sdk 'compatibility/0.5.5')) | Out-Null
foreach ($item in Get-ChildItem -LiteralPath (Join-Path $ordinary 'compatibility/0.5.5')) {
    Copy-Item -LiteralPath $item.FullName -Destination (Join-Path $sdk "compatibility/0.5.5/$($item.Name)") -Recurse -Force
}
...
$release = New-AuthorSdkReleaseInventory -StageRoot $sdk -DotNetSdkVersion '8.0.421' -SdkVersion '0.3.0' -TargetCatalog $catalog
Write-CandidateText (Join-Path $sdk 'author-sdk-release.json') (($release | ConvertTo-Json -Depth 10) + "`n")
$zip = Join-Path $output 'DTMAPI-Author-SDK-0.3.0-candidate-win-x64.zip'
