...
namespace AutoHarvestMod
{
    public sealed class ModEntry : DtmMod
    {
        ...
        private bool inSave;
        private bool automaticUpdateSubscribed;
        ...
        private void OnSaveLoaded(object? sender, SaveLoadedEventArgs e)
        {
            ...
            lastAutoRunSecond = 0;
            RefreshAutomaticUpdater();
            helper.Monitor.Log("AutoHarvest SaveLoaded boundary OK slot=" + (e.SaveSlot?.ToString(CultureInfo.InvariantCulture) ?? "unknown"));
        }
        ...
        private void OnReturnedToTitle(object? sender, ReturnedToTitleEventArgs e)
        {
            inSave = false;
            RefreshAutomaticUpdater();
            helper.Monitor.Log("AutoHarvest ReturnedToTitle boundary OK.");
        }

        private void RefreshAutomaticUpdater()
        {
            bool active = inSave && config.Enabled;
            if (automaticUpdateSubscribed == active)
                return;
            if (active)
                helper.Events.GameLoop.OneSecondUpdateTicked += OnOneSecondUpdateTicked;
            else
                helper.Events.GameLoop.OneSecondUpdateTicked -= OnOneSecondUpdateTicked;
            automaticUpdateSubscribed = active;
        }
