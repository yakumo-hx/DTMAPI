...
- Runtime Workshop 安装器的当前入口、CMD/PowerShell 分层、兼容能力归属和强制包级矩阵只由 [Runtime Workshop Installer Boundary](docs/architecture/runtime-workshop-installer-boundary.md) 与其链接的测试工作流维护；顶层规则和历史 Update 不复制四入口实现细节。
- 功能性 Mod 的玩家 Runtime 来源只有 Doloc Town 官方上传目录
  `LocalLow/RedSawGames/DolocTown/MODS`（`Local.*`）和当前原生订阅快照证明的
  Workshop 安装目录（`Workshop.*`）。历史 `<game>/Mods` 开发目录不参与玩家
  发现。0.6.1 源码候选所保留、但尚未正式发布的 Author SDK 已暂停 `deploy`、`update`、
  `install-local` 和 `source local select`，不得再新建或切换该历史来源；既有
  journal 仍只保留只读 status、`recover`、`withdraw` 与旧选择清理能力，且这些
  兼容操作不能作为可加载或发布验收来源。
...
- **Advanced CodeMod**：显式声明的受管高级 DLL Mod；玩家运行时只从 Doloc Town 官方 `Local.*` 上传目录或当前原生订阅快照证明的 `Workshop.*` 安装目录发现，并接受版本、依赖、排序、诊断、启停/重启和日志管理，同时可以为本产品引用 Unity、Harmony 或游戏程序集并拥有 `ProductNative` 实现。它不得安装到 `BepInEx/plugins`；历史 `<game>/Mods` 仅保留旧部署 recovery/withdraw 兼容，不是发现来源。精确准入产品集、策略和运行证据只由 [Batch 6 Managed Mod Identity Contract](docs/architecture/batch6-managed-mod-identity-contract.md) 及其链接收据维护；任何现有证明都不开放通用 Advanced 作者通道。
...
Batch 6 的精确实现、产品、策略、包、运行与兼容证据由上述契约及其链接的 Update/Review/收据拥有，不在本规范复制快速变化的数量或里程碑状态。Author SDK 的 `SDK160` 继续保护明确声明的 Strict；Advanced manifest、receipt 和 package 必须由 SDK 与受跟踪策略生成，不得手工伪造。任何新增产品、Content Host G7 与未来发布仍须各自的有界授权；历史版本是否完成发布只查其发布 Update/Steam parity 记录，不由 Batch 6 身份证明推导。省略 `CodeModKind` 的 legacy 输入只表示第三方作者自行管理的兼容入口，不等于 SDK-verified Strict 或 receipt-verified Advanced。
