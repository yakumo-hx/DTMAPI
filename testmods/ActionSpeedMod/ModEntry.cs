...
namespace ActionSpeedMod
{
    public sealed class ModEntry : DtmMod
    {
        ...
        private void RegisterConfigMenu(IDtmHelper helper)
        {
            ...
            menu.AddParagraph(helper.ModManifest, () => T(helper, "config.status.toolVerified", "Tool animation, eat/drink animation, bottled-water right-click continuous drink, IWaterContainer and in-water bottle fill, no-key auto-fill, planting, fuel/feed add, plant-basin crop harvest, resin collection, and wild vegetation harvest are verified in smoke."));
            ...
            menu.AddInlineBoolNumberOption(helper.ModManifest, () => T(helper, "config.machineAdd.name", "Interaction speed"), () => T(helper, "config.machineAdd.tooltip", "Fuel/feed, animal, and machine switch interaction animation hook target."), () => config.MachineAddSpeedEnabled, value => config.MachineAddSpeedEnabled = value, () => config.MachineAddMultiplier, value => config.MachineAddMultiplier = value, 1, 4, 0.5);
            ...
            menu.AddInlineBoolNumberOption(helper.ModManifest, () => T(helper, "config.plant.name", "Planting speed"), () => T(helper, "config.plant.tooltip", "Seed, fertilizer, and crop-film interaction animation hook target."), () => config.PlantSpeedEnabled, value => config.PlantSpeedEnabled = value, () => config.PlantMultiplier, value => config.PlantMultiplier = value, 1, 4, 0.5);
