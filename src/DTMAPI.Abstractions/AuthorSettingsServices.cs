...
{
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.6.2", Notes = "Candidate owner configuration; versioned storage is separate from legacy ReadConfig. Runtime thread only.")]
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.6.2", Notes = "Read-only snapshots of existing input routing; conflicts are potential chord overlap, not arbitration. Runtime thread only.")]
    ...
    [DtmApiStatus(DtmApiStatus.Experimental, Since = "0.6.2", Notes = "Parameterized owner translations and synchronous main-thread language notifications. Dispose subscriptions or close owner to detach.")]
