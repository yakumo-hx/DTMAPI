...
public sealed class DoctorEngine
{
        ...
        ICollection<DoctorFinding> findings)
    {
        ...
        bool trackedPolicyMismatch = !advancedReferencePolicy.Matches(receipt, manifest.MinimumDtmApiVersion, out string policyMismatch);
        ...
        ICollection<DoctorFinding> findings)
    {
        ...
        string detail = marker.Error.Length > 0
            ...
            : !receiptIdentity.IsComplete
                ...
                : "marker values do not bind the exact Advanced manifest, entry DLL, reference receipt, and tracked Author SDK target";
        findings.Add(Error(
        ...
        ICollection<DoctorFinding> findings)
    {
        ...
        // These former blocking codes remain named here only as an explicit
        // prohibition for the 0.6 context model: advanced-reference-hash-mismatch,
        // advanced-game-build-unverifiable, advanced-game-build-incompatible.
        // Receipt/package mismatches still fail above; installed-game drift does not.
        identity.ReferenceCompatibility = DoctorCompatibilityStatus.ReceiptVerified;
        bool buildKnown = build.Error.Length == 0;
        bool exact = buildKnown &&
                     build.BuildId.Equals(receipt.GameBuildId, StringComparison.Ordinal) &&
                     driftReferenceCount == 0 &&
                     unavailableReferenceCount == 0 &&
                     exactReferenceCount == receipt.References.Count;
        if (exact)
        {
            identity.GameCompatibility = DoctorCompatibilityStatus.Exact;
            return;
        }

        string installedBuild = buildKnown ? build.BuildId : "unknown";
        string context = "compiledBuild=" + receipt.GameBuildId +
                         "; installedBuild=" + installedBuild +
                         "; references=" + exactReferenceCount + "/" + receipt.References.Count +
                         "; drifted=" + driftReferenceCount +
                         "; unavailable=" + unavailableReferenceCount;
        if (buildKnown)
        {
            identity.GameCompatibility = DoctorCompatibilityStatus.Drift;
            findings.Add(Info(
                "advanced-game-context-drift",
                build.ManifestPath.Length > 0 ? build.ManifestPath : gameRoot,
                "The installed game differs from the receipt compilation baseline; " + context + ".",
                "This is non-blocking compatibility context. Let the Mod run its real type, Entry, Harmony-owner and product activation transaction; update only that product through the Author SDK if activation actually fails."));
        }
        else
        {
            identity.GameCompatibility = DoctorCompatibilityStatus.Unknown;
            findings.Add(Info(
                "advanced-game-context-unknown",
                build.ManifestPath.Length > 0 ? build.ManifestPath : gameRoot,
                "The installed game build mapping is unavailable; " + context + "; buildProbe=" + build.Error + ".",
                "This is non-blocking compatibility context. Do not guess compatibility from the path or rewrite the receipt; let the Mod's real activation transaction decide the product result."));
        }
    }
