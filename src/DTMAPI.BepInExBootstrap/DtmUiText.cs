...
namespace DTMAPI.BepInExBootstrap
{
    internal sealed class DtmUiText
    {
        ...
        private static readonly Dictionary<string, string> SimplifiedChinese = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["item.capture"] = "捕获",
            ["item.clear"] = "清除",
            ["controller.title"] = "输入设置（控制器试验）",
            ["controller.entry"] = "打开 Mod 配置",
            ["controller.nativeEntry"] = "在主菜单显示 Mod 配置入口",
            ["controller.nativeEntryHelp"] = "默认关闭。保存后重启游戏生效；独立图标、快捷键和面板内导航始终可用。",
            ["controller.help"] = "仅标题可打开；游戏中请暂停后返回标题。逗号分隔替代键，加号组合；旁边 + 添加替代键。",
            ["controller.limits"] = "支持 Gamepad 数字按钮。JoystickButtonN 保留原始编号。轴和扳机绑定尚不支持。",
            ["controller.connected"] = "已检测到 Gamepad；实体操作仍需验证。",
            ["controller.absent"] = "未检测到 Gamepad；已存绑定会保留。",
            ["controller.unknown"] = "未识别的按键已保留，请编辑或重置；轴与扳机尚不支持。",
            ["item.none"] = "无",
            ...
            ["commands.label"] = "模组命令（help 查看名称；完整输出见日志）",
            ["commands.run"] = "运行",
            ["commands.queued"] = "命令已排队。",
        ...
        private static readonly Dictionary<string, string> English = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["item.clear"] = "Clear",
            ["controller.title"] = "Input settings (controller experiment)",
            ["controller.entry"] = "Open Mod config",
            ["controller.nativeEntry"] = "Show Mod config in the main menu",
            ["controller.nativeEntryHelp"] = "Off by default. Save and restart the game to apply. The separate icon, shortcut and panel navigation remain available.",
            ["controller.help"] = "Title only. Commas separate alternatives; + joins a chord. Use the + button to add a key.",
            ["controller.limits"] = "Gamepad digital buttons; raw JoystickButtonN. Axis and trigger bindings unsupported.",
            ["controller.connected"] = "Gamepad detected; physical controls await validation.",
            ["controller.absent"] = "No Gamepad detected; saved bindings are retained.",
            ["controller.unknown"] = "Unrecognized button retained; edit or reset. Axes/triggers unsupported.",
            ["item.none"] = "None",
            ...
            ["commands.label"] = "Mod commands (help lists names; full output is in the log)",
            ["commands.run"] = "Run",
            ["commands.queued"] = "Command queued.",
