...
namespace DTMAPI.GameBridge.DolocTown.QA
{
    ...
    internal sealed class QaHostSettings
    {
        ...
        internal void Validate(string expectedRunId)
        {
            ...
            {
                throw new InvalidDataException(
                    "QA MoreEquipmentSlots transition acceptance requires an explicit U1/Prepare/U3Backpack/U3Mail/U4/MigratedSave/ColdPrepare/ColdCommit/ColdObserve phase, the disposable redirect, SaveSlot=3, title lifecycle, NativeSaveExpected for mutation phases, and NoNativeSave for U4/ColdObserve.");
            }
            if (moreEquipmentSlotsTransition &&
                MoreEquipmentSlotsTransitionPhase.Equals(
                    "MigratedSave",
            ...
            string[] allowedG6Cases =
            {
                ...
                "LongTitleLoad",
                "MoreSavesFixed12EnabledLifecycle",
                "MoreSavesFixed12DisabledCold",
                "MoreSavesFixed12ReenabledCold"
            };
            ...
            int exclusiveSaveLoadCases = (G6LifecycleCases.Contains("SaveLoadCycle", StringComparer.Ordinal) ? 1 : 0) +
                ...
                (G6LifecycleCases.Any(item => item.StartsWith("MoreSavesFixed12", StringComparison.Ordinal)) ? 1 : 0);
            if (exclusiveSaveLoadCases > 1)
                throw new InvalidDataException("QA G6 save/load, pending-pressure, long-title, and MoreSaves fixed-12 routes are mutually exclusive.");
            string[] moreSavesFixed12Cases = G6LifecycleCases
                .Where(item => item.StartsWith("MoreSavesFixed12", StringComparison.Ordinal))
                .ToArray();
            if (moreSavesFixed12Cases.Length > 0 && G6LifecycleCases.Length != 1)
                throw new InvalidDataException("A MoreSaves fixed-12 phase must be the single G6 lifecycle case in its cold process.");
            if (moreSavesFixed12Cases.Length > 0)
            {
                string expectedSaveTestMode = moreSavesFixed12Cases[0].Equals(
                    "MoreSavesFixed12EnabledLifecycle",
                    StringComparison.Ordinal)
                        ? "ArchiveMutation"
                        : "NoNativeSave";
                if (SaveSlot != 7 ||
                    !RequireDisposableSaveRedirect ||
                    !SaveTestMode.Equals(
                        expectedSaveTestMode,
                        StringComparison.Ordinal))
                {
                    throw new InvalidDataException(
                        "A MoreSaves fixed-12 phase requires SaveSlot=7, the disposable save redirect, ArchiveMutation for EnabledLifecycle, and NoNativeSave for cold observation.");
                }
            }
