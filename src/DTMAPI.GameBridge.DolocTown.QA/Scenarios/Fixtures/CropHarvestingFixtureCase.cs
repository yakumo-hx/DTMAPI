...
namespace DTMAPI.GameBridge.DolocTown
{
    ...
    {
        private void TryExerciseCropHarvestingApiForFixture()
        {
            const string qaOwnerId = "DTMAPI.QA.G5.CropHarvesting";
            ...
            try
            {
                ...
                if (room == null)
                {
                    runtime.SetHookStatus("Smoke.CropHarvestingApi", "pending", "ICropHarvestingApi", "Waiting for CurrentRoom after SaveLoaded.");
                    return;
                }
                ...
                string summary = "targetId=" + targetId +
                    ", basin={" + DescribeEquipmentForFixture(transientBasin) + " source=" + basinSource + "}" +
                    ", setup={" + setupSummary + "}" +
            ...
            finally
            {
                ...
                CropHarvestingService?.RemoveOwner(qaOwnerId, "G5 fixture terminal cleanup");
            }
        }
        ...
        private bool TryPlantAndMatureTransientCropForHarvestingSmoke(Type dolocApi, object basin, out string setupSummary, out string matureSummary)
        {
            ...
            try
            {
                ...
                {
                    setupSummary = "Could not select plant basin. target=" + DescribeEquipmentForFixture(basin) + ", " + (anchor == null ? "equipment anchor unavailable." : selectSummary);
                    return false;
                }
                ...
                foreach (string seedId in new[] { "seed_endyam", "seed_thunder_grass", "seed_chinese_cabbage", "seed_scallion", "seed_pumpkin", "seed_wheat" })
                {
                    ...
                    {
                        setupSummary = "Could not point item cell tip at plant basin. target=" + DescribeEquipmentForFixture(basin) + ", " + tipSummary;
                        return false;
                    }
                    ...
                    if (crop != null && TrySetCropMatureForFixture(crop, out matureSummary))
                    {
                        setupSummary = "seed=" + seedId +
                            ", seedType=" + ReadSeedTypeForFixture(selectedSeed) +
                ...
                setupSummary = "No seed candidate produced a mature PlantBasin crop. target=" + DescribeEquipmentForFixture(basin) +
                    ", basinSeedType=" + ReadPlantBasinSeedTypeForFixture(basin) +
        ...
        private static ManifestModel CreateCropHarvestingSmokeManifest()
        {
            return new ManifestModel
            {
                ...
                UniqueID = "DTMAPI.QA.G5.CropHarvesting",
                Type = "Smoke"
            };
        }
