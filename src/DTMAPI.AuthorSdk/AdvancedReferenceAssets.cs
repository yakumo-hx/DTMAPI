...
internal static class AdvancedReferenceAssets
{
    ...
    {
        "policyId", "policySha256", "requiredUniqueId", "minimumDtmApiVersion", "compilerSurfaceSha256"
    };
        ...
        string gameRoot)
    {
        ...
        if (CompareNumericVersion(
                manifest.MinimumDTMApiVersion,
                resolved.Registration.MinimumDtmApiVersion) < 0)
        {
            throw new InvalidDataException(
                "Advanced reference policy " +
                resolved.Registration.PolicyId +
                " requires MinimumDTMApiVersion " +
                resolved.Registration.MinimumDtmApiVersion +
                " or later; the packaged manifest declares " +
                manifest.MinimumDTMApiVersion +
                ".");
        }
    ...
    private static void ValidateRegistry(AdvancedReferencePolicyRegistry registry)
    {
        ...
        if (registry.Policies.Any(registration => registration == null
            ...
            || !IsSupportedRuntimeFloor(registration.MinimumDtmApiVersion)))
    ...
    private static bool IsSupportedRuntimeFloor(string value)
    {
        return Version.TryParse(value, out Version? floor)
            && Version.TryParse(AuthorSdkContract.TargetRuntimeVersion, out Version? apiTarget)
            && Version.TryParse(AuthorSdkContract.HighestSupportedRuntimeVersion, out Version? supportedRuntime)
            && floor >= apiTarget
            && floor <= supportedRuntime;
    }
