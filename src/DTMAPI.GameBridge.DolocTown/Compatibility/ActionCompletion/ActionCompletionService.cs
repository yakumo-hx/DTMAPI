#pragma warning disable CS0618 // This file implements the frozen IActionCompletionApi compatibility island.
using System;
...
{
    /// <summary>Frozen 0.5.5 executor for already-built IActionCompletionApi consumers only.</summary>
    internal sealed class ActionCompletionService : IActionCompletionApi
    {
        private const string ManagedProductHarmonyOwner = "dtmapi.mod.yuuka.dtmapi.oneactioncomplete";
        ...
        public void Configure(IManifest owner, ActionCompletionOptions options)
        {
            ...
            ActionCompletionOptions normalized = options ?? new ActionCompletionOptions();
            bool toolRoute = normalized.Enabled &&
                (normalized.CompleteTrees || normalized.CompleteOres || normalized.CompleteGarbage || normalized.CompleteWeeds);
            bool interactExitRoute = normalized.Enabled &&
                (normalized.CompleteMachineFuel || normalized.CompleteFeeder);
            if (toolRoute || interactExitRoute)
                ThrowIfManagedProductOwnsActionRoute();
            actionOptions[owner.UniqueID] = normalized;
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.ActionCompletion, owner.UniqueID, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "policy", toolRoute || interactExitRoute, "action completion callback closure configured");
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.ToolColliderShared, owner.UniqueID, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "action-completion-tool", toolRoute, "action completion ToolCollider dependency");
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.AgentStateInteractExitShared, owner.UniqueID, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "action-completion-interact-exit", interactExitRoute, "action completion AgentStateInteract.OnExit dependency");
        ...
        {
            if (actionOptions.Count == 0 || !managedProductOwnerPresent())
                return 0;

            var owners = new List<string>(actionOptions.Keys);
            int removed = 0;
            foreach (string ownerId in owners)
                removed += RemoveOwner(ownerId, "managed OneActionComplete product acquired native ownership before compatibility Hook installation");

            actionHooksInstalled = false;
            runtime.RuntimeMonitor.Log("Frozen IActionCompletionApi compatibility removed " + removed + " pending owner(s) because the managed OneActionComplete product owns the action-completion Hooks.");
            return removed;
        }

        private void ThrowIfManagedProductOwnsActionRoute()
        {
            if (managedProductOwnerPresent())
                throw new InvalidOperationException("Frozen IActionCompletionApi compatibility refused action-completion ownership because the managed OneActionComplete product is active.");
        }

        private static bool IsManagedProductOwnerPresent()
        {
            return TargetHasManagedProductOwner("DolocTown.ToolCollider, Assembly-CSharp", "HandleTools", 1) ||
                TargetHasManagedProductOwner("DolocTown.AgentStateInteract, Assembly-CSharp", "OnExit", 0);
        }

        private static bool TargetHasManagedProductOwner(string typeName, string methodName, int parameterCount)
        {
            Type? type = ResolveType(typeName);
            MethodInfo? target = FindMethodInHierarchy(type, methodName, parameterCount);
            ...
            {
                string owner = ownerValue as string ?? string.Empty;
                if (owner.Equals(ManagedProductHarmonyOwner, StringComparison.Ordinal))
                    return true;
            }
            return false;
        }
        ...
        internal int RemoveOwner(string ownerId, string reason)
        {
            ...
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.ActionCompletion, ownerId, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "policy", false, "action completion owner cleanup " + (reason ?? string.Empty));
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.ToolColliderShared, ownerId, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "action-completion-tool", false, "action completion owner cleanup " + (reason ?? string.Empty));
            GameBridgeDemandRoutes.SetOwnerDemand(runtime, GameBridgeDemandRoutes.AgentStateInteractExitShared, ownerId, RuntimeDemandSourceType.CapabilityRegistration, RuntimeDemandLifetime.Owner, "action-completion-interact-exit", false, "action completion owner cleanup " + (reason ?? string.Empty));
            if (!actionOptions.Remove(ownerId))
                return 0;
            loggedActionApplications.RemoveWhere(key => key.IndexOf(ownerId, StringComparison.OrdinalIgnoreCase) >= 0);
            return 1;
        }

        internal int CountOwnerResources(string ownerId) => actionOptions.ContainsKey(ownerId ?? string.Empty) ? 1 : 0;
